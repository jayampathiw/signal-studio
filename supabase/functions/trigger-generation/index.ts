...
Deno.serve(async (req: Request) => {
  ...
  if (!githubPat) return json({ error: 'GITHUB_PAT secret not configured — add it via Supabase Dashboard → Edge Functions → Secrets' }, 500);
  ...
  if (item.status !== 'storyboard') {
    return json({ error: `Expected status 'storyboard', got '${item.status}'. Save your scene edits first.` }, 422);
  }
