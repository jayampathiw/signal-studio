...
export async function insertContentItem(row) {
  const { data, error } = await getClient().from('content_items').insert(row).select().single();
  if (error) throw error;
  return data;
}

export async function getContentItem(id) {
  const { data, error } = await getClient().from('content_items').select('*').eq('id', id).single();
  if (error) throw error;
  return data;
}
