...
export const IntroOutroCard: React.FC<Props> = ({ wordmark = 'THE POLICY FILE' }) => {
  ...
  const highlightOpacity = interpolate(frame, [35, 55], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const textOpacity = interpolate(frame, [45, 65], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });

  return (
    <AbsoluteFill style={{ backgroundColor: PALETTE.background, justifyContent: 'center', alignItems: 'center' }}>
      <div style={{ position: 'relative', width: 360, height: 360, transform: `translateY(${foldEnter}px)` }}>
        {/* Back tab */}
        <div style={{ position: 'absolute', left: 60, top: 70, width: 60, height: 140, background: PALETTE.backgroundShadow }} />
        <div style={{ position: 'absolute', right: 60, top: 70, width: 60, height: 140, background: PALETTE.backgroundShadow }} />
        ...
        >
          <div style={{ width: '85%', height: 10, background: PALETTE.bodyLine, marginBottom: 10 }} />
          <div style={{ width: '65%', height: 10, background: PALETTE.bodyLine, marginBottom: 10 }} />
          <div style={{ width: '75%', height: 12, background: PALETTE.highlight, opacity: highlightOpacity }} />
        </div>
      ...
      <p style={{ fontFamily: serifFont, fontWeight: 700, fontSize: 52, color: PALETTE.paper, margin: '32px 0 8px', opacity: textOpacity }}>
        {wordmark}
      </p>
      <p style={{ fontFamily: monoFont, fontSize: 22, color: PALETTE.taglineGold, letterSpacing: 2, opacity: textOpacity }}>
        {TAGLINE}
      </p>
    </AbsoluteFill>
  );
