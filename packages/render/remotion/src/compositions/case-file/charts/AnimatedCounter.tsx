...
export const AnimatedCounter: React.FC<{ visual: CounterVisual; scale?: number }> = ({ visual, scale = 1 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const countFrames = 1.5 * fps;

  const displayed = Math.round(interpolate(frame, [0, countFrames], [0, visual.value], {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: PALETTE.background, justifyContent: 'center', alignItems: 'center', opacity }}>
      <p style={{ fontFamily: serifFont, fontWeight: 700, fontSize: 260 * finalScale, color: PALETTE.highlight, margin: 0, lineHeight: 1 }}>
        {visual.prefix ?? ''}{displayed}{visual.suffix ?? ''}
      </p>
      {visual.label && (
        <p style={{
          fontFamily: monoFont, fontWeight: 700, fontSize: 44, lineHeight: 1.4, color: PALETTE.paper,
          marginTop: 28, textAlign: 'center', maxWidth: 840,
        }}>
          {visual.label}
        </p>
      )}
