...
- [~] **P2.6 Docker image** (2026-09-19)
  ...
  - [x] `.github/workflows/image.yml` — builds and pushes `ghcr.io/<owner>/signal-studio-worker:{sha,dev}` on push to `main`/`refactor` and on `v*` tags, via `docker/build-push-action` with GHA layer caching. **Not yet exercised end-to-end** — needs a real push to trigger it, not attempted this session. Includes a best-effort `gh api` step to set the ghcr.io package to private (this repo is public, so a package pushed via `GITHUB_TOKEN` defaults to public visibility, and the plan calls for private) — `continue-on-error: true` since `GITHUB_TOKEN` may not carry enough scope to change a user-owned package's overall visibility setting, only to publish/delete versions of it; unverified either way until a real run happens
