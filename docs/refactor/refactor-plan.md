...
- [ ] **P0.1 Golden references**
  - [ ] `scripts/golden/render-reference.mjs`: runs today's script for a given template and copies the output to `golden/<template>/reference.mp4`
  - [ ] Render references: Policy File 16:9 (`assemble-case.mjs` on one existing case), Policy File 9:16 (`CaseFileVertical`), one Underdog long-form act (`assemble-local.mjs --scenes 1-8` on a shipped project), one Short (`assemble-short-rewrite.mjs` on a shipped config), the P0.8 pilot `_fb` + `_ig`
  - [ ] `scripts/golden/measure.mjs`: writes `golden.json` = duration, width/height/fps, integrated LUFS + true peak (`loudnorm print_format=json`), scene-cut timestamps (`select='gt(scene,0.4)'`), pHash of 10 evenly spaced frames
  - [ ] `scripts/golden/check.mjs <mp4> <golden.json>`: tolerances — duration ±0.15 s, LUFS ±0.5, TP ≤ −1.0, scene cuts ±0.2 s, pHash Hamming distance ≤ 8 on ≥ 8/10 frames; exit 1 on failure with a table
  - [ ] Upload references + `golden.json` to R2 `golden/`; commit only the `golden.json` files under `golden/`
  - [ ] Unit test for `check.mjs` using two tiny synthetic MP4s (pass and fail)
- [ ] **P0.2 CI baseline**
  - [ ] `eslint.config.js` (flat): base + `no-console` (error), import order; TS files via `typescript-eslint`
  - [ ] `.prettierrc` + `.prettierignore`; `npm run format` / `format:check`
  - [ ] `tsconfig.base.json` with `checkJs: true`, `strict: true`; per-package `tsconfig.json` extends it
  - [ ] `vitest.config.ts` at root with workspace projects; `npm test`
  - [ ] `.github/workflows/ci.yml`: install → lint → format:check → typecheck → test; Node 22; cache npm
  - [ ] Branch protection on `refactor/signal-studio`: CI required
  - [ ] Per-file `/* eslint-disable no-console */` allow-list on existing files, tracked in `docs/PROJECT-STATUS.md` as a burn-down count
- [ ] **P0.3 Hooks**
  - [ ] Create `scripts/hooks/env-check.js` (checks `.env` exists and required keys are non-empty; exit 0 with a warning, never blocks)
  - [ ] Remove the repo-wide `safe-language-lint` PostToolUse hooks from `.claude/settings.json`
  - [ ] Add PostToolUse hook: `eslint --fix` + `prettier --write` on the edited file (Write/Edit matchers)
  - [ ] Verify with `/hooks` in a Claude Code session; run one Bash command and one Edit to confirm no failures
