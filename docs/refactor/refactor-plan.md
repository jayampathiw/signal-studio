...
| P2 — `clips-overlay` + M1 runtime  | 72%     | 49 / 13 / 15          |
...
| **Overall (weighted)**             | **51%** | **190.5 / 371**       |

The three phases finished first (P0.8, P0, P1) are 76–86% each; P2 is now well underway — providers (P2.4), the adapter + assets stage (P2.3), the Docker image (P2.6), both templates' `compile()` + Remotion compositions (P2.1 `clips-overlay`, P2.2 `compilation`), and now `apps/worker`'s full `ss` CLI (P2.5 — `validate`/`run-local`/`run-job`/`upload`/`worker`) are done, each individually verified with a real render, real external call, or the real CLI binary itself. **A significant repo-wide bug was found and fixed while building P2.5**: most packages' automated test scripts were silently never running a large fraction of their own tests under `pnpm run test`/CI (a shell-globbing gap, not a code defect) — full details and the fix are in P2.5's own entry; every test that had been silently skipped was confirmed already-passing once actually run. `apps/api` (P2.7 — jobs CRUD, presign, approve, dispatch) is built and verified for real against the live `dev` Supabase and R2; you applied the `api_keys` migration and set the GH Actions secrets it needed. P2.8 (AssembleX project config + seeding) is done too, and its real end-to-end `ss upload` → `ss run-job` run against the live DB genuinely walked a job from `created` to `delivered` on its own — real renders, real R2 uploads, real artifact rows. **That same real run surfaced another real bug**, this time in `job_stages`' unique constraint (never enforced correctly for single-output stages, invisible until tested against a real Postgres) — fixed, migration written, not yet applied. Two small things remain open: applying that new migration, and `api_keys` writes still hitting a PostgREST schema-cache staleness error even after the table itself was confirmed to exist. What's left in P2 is its own T-L→T-G→T-P test gates. Phases 3–6 and Track C remain untouched. As of 2026-09-22.
...
- [~] **P2.7 `run-job.yml` + API (GitHub dispatch mode)** (2026-09-22)
  ...
  - [x] API-key middleware (`src/middleware/api-key.ts`) — `Authorization: Bearer <key>` → sha256 hash → `ApiKeysRepo.findByHash()` → `c.set('orgId', ...)`. New `api_keys` table + `ApiKeysRepo` (`packages/db/src/repos/api-keys.ts`, hashed storage, raw key returned exactly once at creation — never persisted). **New migration not yet applied**: `packages/db/migrations/20260922090000_api_keys.sql` — this sandbox's session doesn't have the Supabase MCP server connected (the same one P1.5's migration went through; needs `SUPABASE_MCP_TOKEN`), so there's no path to apply it here. Confirmed the table genuinely doesn't exist yet by querying the live dev project directly (`Could not find the table 'public.api_keys'`) rather than assuming. `api-keys.ts`'s and the middleware's own tests are fakes-only until it's applied.
...
- [~] **P2.8 AssembleX project on the engine** (2026-09-22)
  ...
  - [~] `api_keys` migration (flagged as unapplied in P2.7's own entry) — **you applied it** (confirmed: table existence check went from erroring to succeeding for `SELECT`). **Still incomplete**: `INSERT` against `api_keys` still fails with `Could not find the table 'public.api_keys' in the schema cache` even after a `NOTIFY pgrst, 'reload schema'` attempt and some elapsed time — a known Supabase/PostgREST staleness quirk after DDL changes, not a bug in the migration's SQL itself (confirmed the table and its columns are real via `SELECT`), but it hasn't self-resolved yet. Re-check later or restart the project's API from the Supabase dashboard (Project Settings → General → Restart project) if `NOTIFY` alone doesn't clear it.
