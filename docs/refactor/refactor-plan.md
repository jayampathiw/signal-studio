...
- [~] **P0.1 Golden references** (2026-09-18) — tooling built and verified; 5/6 references pass their own golden.json, 1/6 (the Short) fails on a real, pre-existing true-peak violation the tool itself caught; R2 upload blocked on missing credentials
  ...
  - [~] Upload references + `golden.json` to R2 `golden/` — **not attempted**: `.env`'s `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_PUBLIC_BASE_URL` are all empty (only the two bucket-name placeholders are set), so an upload would just fail. Commit only the `golden.json` files under `golden/` — **done**: `golden/**/*.mp4` added to `.gitignore` (same reasoning as `content/`'s move out in P0.4 — real production-adjacent media doesn't belong in this repo's history), all 6 reference `.mp4`s (649MB total) exist locally only, not committed, not yet backed up off this machine. Needs real R2 credentials before this step can close
  - **Also surfaced, not fixed here** (flagged for whoever picks up P0.4/P0.6 follow-through, or a dedicated pass): the Underdog long-form/Shorts `stills/` directories are `.gitignore`'d in this repo (`content/longform/*/stills/`, "regenerable from configs + source stills") and were consequently **never copied** into `signal-studio-workspace` by P0.4's `content/` move, even though P0.4's own commit message implied a full copy — they only still exist because a separate `main` git worktree on this machine happens to retain the pre-P0.4 `content/` tree on disk. If that worktree is ever cleaned up, the stills are gone entirely, workspace repo included. This wasn't "regenerable from configs" so much as machine-local, ungitignored-by-mistake production content
- [~] **P0.2 CI baseline** (2026-09-18) — local (T-L) done; T-G (push + branch protection) deliberately not attempted, see below
  ...
  - [ ] Branch protection on `refactor/signal-studio` — **not attempted**: (a) the branch is actually named `refactor` here, not `refactor/signal-studio` (naming mismatch noted since the first Phase 0.8 session, still unreconciled); (b) this is a GitHub repo-admin setting on shared state — you chose "commit locally only, don't push" for this round, so `ci.yml` hasn't run on GitHub yet either. Needs you to push and set this up, or tell me to.
