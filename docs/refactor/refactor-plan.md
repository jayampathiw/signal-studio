...
- [~] **P2.7 `run-job.yml` + API (GitHub dispatch mode)** (2026-09-22)
  ...
  - [~] `GITHUB_PAT` for the dispatcher's real GitHub call — **not set locally** (confirmed empty in `.env`, a pre-existing gap this repo's own docs already track under "four separate secret stores"). The dispatcher's HTTP call shape itself is proven correct via `dispatcher.test.ts`'s 3 tests (real URL, real headers, real body shape, real race-skip, real error path) against a fake `fetch`; a genuine `workflow_dispatch` call — and the GH Actions secrets `run-job.yml` needs (`ENGINE_SUPABASE_*`, `R2_BUCKET_RENDERED` aren't yet set as repo secrets either, only present locally) — are flagged for you rather than pushed to the repo's shared GitHub configuration without asking first.
