...
| P2 — `clips-overlay` + M1 runtime  | 49%     | 29 / 6 / 31           |
...
| **Overall (weighted)**             | **46%** | **167 / 360**         |

The three phases finished first (P0.8, P0, P1) are 76–86% each; P2 is now underway too — providers (P2.4), the adapter + assets stage (P2.3), the Docker image (P2.6), and both templates' `compile()` + Remotion compositions (P2.1 `clips-overlay`, P2.2 `compilation`) are done and each individually verified with a real render or real external call, not just unit tests. What's left in P2 (`apps/worker` P2.5, the API/dispatch P2.7, and P2's own T-L→T-G→T-P gates) is genuinely sequential — each needs the piece before it. Phases 3–6 and Track C remain untouched. As of 2026-09-22.
...
- [~] **P2.4 Providers** (2026-09-19)
  ...
  - [x] Contract tests pass for each — 18 tests always run (fakes/mocks, no network or real models, CI-safe); 4 more are real-only and skipped unless `PROVIDERS_REAL=1` (`pnpm --filter @signal-studio/providers run test:real`) — 3 of those 4 (1 `storage-r2`, 2 `tts-kokoro-js`) were actually run for real in this session (see above); the remaining 1 (`tts-kokoro-py`) stays skip-only pending a python `kokoro` env
- [ ] **P2.5 `apps/worker`**
