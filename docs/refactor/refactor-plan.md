...
| P2 — `clips-overlay` + M1 runtime  | 74%     | 52 / 11 / 15          |
...
| **Overall (weighted)**             | **52%** | **192.5 / 372**       |

The three phases finished first (P0.8, P0, P1) are 76–86% each; P2 is now well underway — providers (P2.4), the adapter + assets stage (P2.3), the Docker image (P2.6), both templates' `compile()` + Remotion compositions (P2.1 `clips-overlay`, P2.2 `compilation`), and now `apps/worker`'s full `ss` CLI (P2.5 — `validate`/`run-local`/`run-job`/`upload`/`worker`) are done, each individually verified with a real render, real external call, or the real CLI binary itself. **A significant repo-wide bug was found and fixed while building P2.5**: most packages' automated test scripts were silently never running a large fraction of their own tests under `pnpm run test`/CI (a shell-globbing gap, not a code defect) — full details and the fix are in P2.5's own entry; every test that had been silently skipped was confirmed already-passing once actually run. `apps/api` (P2.7 — jobs CRUD, presign, approve, dispatch) and P2.8 (AssembleX project config + seeding) are both fully done now, proven with the most complete real chain yet: a real `ss upload` → `ss run-job` walked a real job from `created` to `delivered` entirely on its own (real renders, real R2 uploads), and a real `apps/api` server answered real HTTP requests — auth-gated with a real API key, 401 without one — returning that same real job. Two real bugs surfaced and got fixed along the way: `job_stages`' unique constraint never actually worked for single-output stages (SQL `NULL <> NULL`), and this repo's migration filenames didn't follow Supabase's required timestamp convention, which was silently masking a third issue — the `api_keys` table had actually never been created at all (an earlier "it exists" check was a false positive from a quirky PostgREST response, not a caching delay as first assumed). All fixed and verified for real, not just believed fixed. What's left in P2 is its own T-L→T-G→T-P test gates. Phases 3–6 and Track C remain untouched. As of 2026-09-22.
