...
| # | Finding | Impact |
|---|---|---|
| F1 | **Bug:** `generate-tts.mjs:76` writes `content_clips.status_note` — that column doesn't exist on `content_clips` (it's `fail_reason`; `status_note` lives on `content_items`). The TTS *error path* itself throws | P0-2 |
...
2. **YouTube OAuth** — Google Cloud project + OAuth client + one-time consent flow (P12; only blocks the *publish* stage, nothing earlier).
...
### P0 — Preflight: fixes + ground truth *(no new features)*
