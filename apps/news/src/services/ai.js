...
Résumé : ${summary}`,
  ...
  try {
    const rawText = (typeof response === 'string' ? response : response.content[0].text).trim();
    ...
    return {
      intro: typeof response === 'string' ? response : response.content[0].text.trim(),
      question: '',
      cta: '',
      hashtags: [],
...
Résumé: ${summary}`,
  ...
  let parsed;
  try {
    const text = (typeof response === 'string' ? response : response.content[0].text).trim();
...
Return ONLY valid JSON, no markdown, starting with {:
{
  ...
  try {
    ...
    throw new Error(`Claude returned invalid JSON for On This Day: ${text.slice(0, 200)}`);
  }
}
