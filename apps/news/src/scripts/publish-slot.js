...
import {
  getPendingArticlesSortedByScore,
  getFirstBoostIneligiblePostedIT,
  updateArticle,
...
if (missing.length) {
  console.error(
    `PUBLISH-SLOT FAILED: Missing required environment variables: ${missing.join(', ')}`,
  );
  process.exit(1);
}
...
async function run() {
  ...
  for (const country of ['IT', 'FR']) {
    ...
    if (!inWindow) {
      console.log(
        `[${country}] Not in slot window (nearest: ${closest} from [${slots.join(', ')}])`,
      );
      continue;
    }
    console.log(
      `[${country}] In slot window for ${closest} CEST (today's slots: ${slots.join(', ')})`,
    );
...
async function publishForCountry(country, slotTarget) {
  ...
  console.log(
    `[${country}] slot=${slotTarget} article=${article.id} score=${article.publish_score?.toFixed(1)} runner_up_score=${runnerUp?.publish_score?.toFixed(1) ?? 'none'}`,
  );
  ...
  try {
    let fbPostId;
    const captionText =
      [article.ai_caption.intro, article.ai_caption.question, article.ai_caption.cta]
        .filter(Boolean)
        .join('\n\n') ||
    ...
    console.log(
      `[${country}] SLOT_POST slot_target=${slotTarget} actual_fire_time=${postedAt} article_id=${article.id} publish_score=${article.publish_score?.toFixed(1)} runner_up=${runnerUp?.id ?? 'none'} runner_up_score=${runnerUp?.publish_score?.toFixed(1) ?? 'none'}`,
    );
