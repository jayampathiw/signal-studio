...
const VIDEO_PROMPT_FIELDS: { key: string; label: string; rows: number; hint: string }[] = [
  {
    ...
    label: 'Composition',
    rows: 2,
    hint: 'Framing, subject placement, orientation',
  },
  {
    key: 'style',
    label: 'Style',
    rows: 1,
    hint: 'ultrarealistic, wildlife documentary cinematography, film grain',
  },
  {
    key: 'cameraMotion',
    label: 'Camera Motion',
    rows: 2,
    hint: 'Named move + emotional justification',
  },
  {
    key: 'subjects',
    label: 'Subjects',
    rows: 2,
    hint: 'Species detail, fur/eye description, props',
  },
  { key: 'action', label: 'Action', rows: 2, hint: 'Micro-events, NOT macro-drama' },
  {
    ...
    label: 'Location',
    rows: 2,
    hint: 'Interior/exterior, specific setting detail',
  },
  {
    key: 'audioCues',
    label: 'Audio Cues',
    rows: 2,
    hint: 'Specific layered sounds — never vague. Natural audio ONLY.',
  },
  {
    key: 'lighting',
    label: 'Lighting',
    rows: 2,
    hint: 'Always directional, always emotionally purposeful',
  },
];
...
const SCENARIO_OPTIONS = [
  {
    value: 1,
    label: '1',
    desc: 'Storyboard-direct — no start frame, uses storyboard panel as video reference.',
  },
  {
    value: 2,
    label: '2',
    desc: 'Fresh start frame — previous final frame = CHARACTER reference only.',
  },
...
@Component({
    ...
    >
      ...
      >
        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
          <span
            [class]="'ink-badge ' + statusBadgeClass(item.status)"
            style="text-transform:uppercase;"
            >{{ item.status }}</span
          >
          @if (item.format) {
            <span class="ink-badge ib-brand" style="font-family:'JetBrains Mono',monospace;">{{
              item.format
            }}</span>
          }
          @if (item.slot) {
            <span
              style="font-size:10px;color:var(--ink-text-2);font-family:'JetBrains Mono',monospace;"
              >{{ item.slot }}</span
            >
          }
        </div>
        <div style="display:flex;align-items:center;gap:8px;">
          <span
            class="ink-badge"
            style="background:var(--ink-raised);color:var(--ink-text-2);font-family:'JetBrains Mono',monospace;font-size:9px;"
            >#{{ item.id }}</span
          >
      ...
      <div style="padding:10px 16px;border-bottom:1px solid var(--ink-border);flex-shrink:0;">
        <h2
          style="font-size:14px;font-weight:600;line-height:1.4;color:var(--ink-text);margin:0 0 4px;"
        >
          {{ item.title ?? item.channel_key }}
        </h2>
        <div style="font-size:11px;color:var(--ink-text-2);font-family:'JetBrains Mono',monospace;">
          {{ item.channel_key }}
        </div>
      </div>
      ...
      <div class="ink-tabs scrollbar-none" style="overflow-x:auto;flex-shrink:0;">
        @for (tab of tabs; track tab; let i = $index) {
          ...
          >
            ...
            @if (i === 1 && dirtyCount() > 0) {
              <span
                style="display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;border-radius:50%;background:var(--ink-alert);color:#fff;font-size:8px;font-weight:700;margin-left:4px;"
                >{{ dirtyCount() }}</span
              >
            }
            @if (i === 2 && genConfigDirty()) {
              <span
                style="display:inline-flex;align-items:center;justify-content:center;width:6px;height:6px;border-radius:50%;background:var(--ink-alert);margin-left:4px;"
              ></span>
            }
          </button>
        }
      ...
      <div style="padding:16px;display:flex;flex-direction:column;gap:16px;">
        ...
        @if (activeTab() === 0) {
          @if (item.status_note) {
            <div
              [style.background]="
                item.status === 'blocked' ? 'rgba(255,54,54,.08)' : 'rgba(255,140,0,.08)'
              "
              [style.border]="
                item.status === 'blocked'
                  ? '1px solid rgba(255,54,54,.25)'
            ...
            >
              <p
                class="section-label"
                [style.color]="
                  item.status === 'blocked' ? 'var(--ink-breaking)' : 'var(--ink-alert)'
                "
                style="margin-bottom:4px;"
              >
                {{ item.status === 'blocked' ? '⛔ Blocked' : '⚠ Note' }}
              </p>
              <p style="font-size:12px;line-height:1.6;margin:0;color:var(--ink-text);">
                {{ item.status_note }}
              </p>
            </div>
          }
          ...
          @if (item.status === 'brief' && !item.scenes?.length) {
            <div
              style="border:1px dashed var(--ink-brand);border-radius:10px;padding:18px 16px;display:flex;flex-direction:column;align-items:center;gap:10px;text-align:center;background:var(--ink-brand-glow);"
            >
              <div style="font-size:28px;line-height:1;">✦</div>
              <p style="font-size:13px;font-weight:600;color:var(--ink-text);margin:0;">
                Ready to expand this brief
              </p>
              <p
                style="font-size:12px;color:var(--ink-text-2);margin:0;line-height:1.5;max-width:340px;"
              >
                Claude will read the concept title and generate full image + video prompts for each
                scene. You can review and edit them before running Higgsfield.
              </p>
              <div
                style="display:flex;gap:6px;align-items:center;font-size:11px;color:var(--ink-text-3);"
              >
                ...
                <span
                  >{{ item.format ?? '11s' }} ·
                  {{ (item.format ?? '11s') === '21s' ? '3 scenes' : '1 scene' }}</span
                >
              </div>
              <button
                class="btn-brand"
                style="padding:0 20px;height:38px;font-size:13px;gap:8px;"
          ...
          @if (item.status === 'storyboard') {
            <div
              style="border:1px solid var(--ink-brand);border-radius:10px;padding:16px;display:flex;flex-direction:column;gap:10px;background:var(--ink-brand-glow);"
            >
              <div style="display:flex;align-items:center;gap:8px;">
                <span style="font-size:18px;line-height:1;">▶</span>
                <p style="font-size:13px;font-weight:600;color:var(--ink-text);margin:0;">
                  Scene prompts ready — run the pipeline
                </p>
              </div>
              <p style="font-size:12px;color:var(--ink-text-2);margin:0;line-height:1.5;">
                Dispatches a GitHub Actions runner that calls the Wild Eye skill: Higgsfield start
                frames → vision gate → video generation → SEO. Takes ~10–20 min.
              </p>
              <div
              ...
              >
                ...
                >
                  <span class="ink-badge ib-ai">storyboard</span>
                  <span>→</span>
                  <span class="ink-badge" style="background:rgba(255,140,0,.15);color:#ff8c00;"
                    >generating</span
                  >
                  <span>→</span>
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                  @if (runUrl()) {
                    <a
                      [href]="runUrl()!"
                      target="_blank"
                      style="font-size:11px;color:var(--ink-brand);text-decoration:none;font-weight:600;"
                      >View Actions run →</a
                    >
                  }
                  <button
                    class="btn-brand"
                    style="height:34px;padding:0 16px;font-size:12px;gap:8px;"
                    [disabled]="triggering()"
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
            ...
            <div>
              ...
              <span class="ink-badge" style="font-size:10px;text-transform:uppercase;">{{
                item.style ?? '—'
              }}</span>
            </div>
          </div>
          ...
          @if (item.slot) {
            <div>
              <p class="section-label">Publish Slot</p>
              <p
                style="font-size:14px;font-weight:600;color:var(--ink-text);font-family:'JetBrains Mono',monospace;"
              >
                {{ item.slot }}
              </p>
            </div>
          ...
          <div>
            ...
            <p style="font-size:13px;color:var(--ink-text);font-family:'JetBrains Mono',monospace;">
          ...
          <div>
            ...
            <div style="display:flex;align-items:center;gap:8px;">
              <code
                style="font-family:'JetBrains Mono',monospace;font-size:13px;color:var(--ink-text-2);background:var(--ink-raised);border:1px solid var(--ink-border);padding:4px 8px;border-radius:4px;flex:1;"
                >#{{ item.id }}</code
              >
              <button
                class="btn-ink"
                style="height:28px;padding:0 10px;font-size:11px;flex-shrink:0;"
                (click)="copy(item.id.toString())"
              >
                Copy
              </button>
            </div>
        ...
        @if (activeTab() === 1) {
          @if (dirtyCount() > 1) {
            <div
              style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:rgba(255,140,0,.08);border:1px solid rgba(255,140,0,.25);border-radius:8px;"
            >
              <span style="font-size:12px;color:var(--ink-alert);font-weight:600;"
                >{{ dirtyCount() }} scenes have unsaved changes</span
              >
              <button
                class="btn-brand"
                style="background:var(--ink-alert);height:28px;font-size:11px;padding:0 12px;"
                [disabled]="saving()"
                (click)="saveAll()"
              >
          ...
          @if (!sceneDrafts().length) {
            @if (item.status === 'brief') {
              <div
                style="display:flex;flex-direction:column;align-items:center;gap:12px;padding:32px 16px;text-align:center;"
              >
                <div style="font-size:36px;opacity:.25;">✦</div>
                <p
                  style="font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:var(--ink-text-3);"
                >
                  Brief not expanded yet
                </p>
                <p style="font-size:12px;color:var(--ink-text-2);line-height:1.5;max-width:320px;">
                  Generate scene prompts from the concept title first. You can edit them here before
                  triggering image generation.
                </p>
                <button
                  class="btn-brand"
                  style="height:36px;padding:0 18px;font-size:12px;gap:8px;"
                  [disabled]="expandingBrief()"
                  (click)="expandBrief()"
                >
                  @if (expandingBrief()) {
              ...
              <div class="empty-state">
                ...
                <p style="font-size:11px;color:var(--ink-text-3);margin-top:4px;">
                  Run the
                  <code
                    style="font-family:'JetBrains Mono',monospace;background:var(--ink-raised);padding:2px 5px;border-radius:3px;"
                    >wild-eye-reel</code
                  >
                  skill to start generation
                </p>
              </div>
            }
          }
          ...
          @for (scene of sceneDrafts(); track scene.scene_num; let idx = $index) {
            <div style="border:1px solid var(--ink-border);border-radius:10px;overflow:hidden;">
              <!-- Scene header bar -->
              <div
                style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--ink-border);flex-wrap:wrap;background:var(--ink-surface);"
              >
                <span class="ink-badge" style="font-weight:700;font-size:10px;"
                  >Scene {{ scene.scene_num }}</span
                >
                @if (scene.scene_status) {
                  <span
                    [class]="'ink-badge ' + sceneStatusClass(scene.scene_status)"
                    style="font-size:9px;"
                    >{{ scene.scene_status }}</span
                  >
                }
                <span style="flex:1;"></span>
                <div style="display:flex;align-items:center;gap:4px;">
                  <label
                    style="font-size:9px;color:var(--ink-text-3);text-transform:uppercase;letter-spacing:.06em;white-space:nowrap;"
                    >Dur.</label
                  >
                  <input
                    type="number"
                    class="ink-input"
                    style="width:50px;height:26px;padding:0 6px;font-size:12px;font-family:'JetBrains Mono',monospace;text-align:center;"
                ...
                <div style="display:flex;align-items:center;gap:4px;">
                  <label
                    style="font-size:9px;color:var(--ink-text-3);text-transform:uppercase;letter-spacing:.06em;"
                    >Scenario</label
                  >
                  <select
                    class="ink-select"
                    style="height:26px;font-size:11px;padding:0 6px;min-width:46px;"
                ...
                <div style="display:flex;align-items:center;gap:4px;">
                  ...
                  <select
                    class="ink-select"
                    style="height:26px;font-size:11px;padding:0 6px;min-width:58px;"
              ...
              @if (scene.scenario) {
                <div
                  style="padding:5px 12px;font-size:10px;color:var(--ink-text-3);background:var(--ink-dim);border-bottom:1px solid var(--ink-border);line-height:1.4;"
                >
              ...
              @if (scene.start_frame_url || scene.storyboard_url || scene.clip_url) {
                <div
                  style="display:flex;gap:8px;padding:10px 12px;border-bottom:1px solid var(--ink-border);overflow-x:auto;align-items:flex-start;background:var(--ink-raised);"
                >
                  @if (scene.start_frame_url || scene.storyboard_url) {
                    <div style="position:relative;flex-shrink:0;">
                      <img
                        [src]="scene.start_frame_url ?? scene.storyboard_url!"
                        style="height:110px;width:auto;border-radius:6px;display:block;background:#000;border:1px solid var(--ink-border);"
                      />
                      <div style="position:absolute;top:4px;left:4px;">
                        <span
                          class="ink-badge"
                          style="font-size:8px;opacity:.88;backdrop-filter:blur(6px);"
                          >{{ scene.start_frame_url ? 'start frame' : 'storyboard' }}</span
                        >
                      </div>
                      @if (scene.vision_check) {
                        <div style="position:absolute;top:4px;right:4px;">
                          <span
                            [class]="'ink-badge ' + visionBadgeClass(scene.vision_check.status)"
                            style="font-size:9px;backdrop-filter:blur(6px);"
                          >
                  ...
                  @if (scene.clip_url) {
                    <div style="flex-shrink:0;">
                      <video
                        ...
                        controls
                        preload="metadata"
                        style="height:110px;width:auto;border-radius:6px;background:#000;display:block;border:1px solid var(--ink-border);"
                      ></video>
                    </div>
                  }
                  @if (scene.vision_check?.issues?.length) {
                    <div
                      style="display:flex;flex-direction:column;gap:3px;padding:4px 0;flex-shrink:0;max-width:190px;"
                    >
                      @for (issue of scene.vision_check!.issues; track issue) {
                        <p
                          style="font-size:10px;color:var(--ink-breaking);margin:0;line-height:1.4;"
                        >
                          • {{ issue }}
                        </p>
                      }
              ...
              <!-- Prompt editors -->
              <div
                style="padding:14px 12px;display:flex;flex-direction:column;gap:16px;background:var(--ink-raised);"
              >
                <!-- Image Prompt -->
                <div>
                  <div
                  ...
                  >
                    ...
                    <span
                      style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-3);"
                      >{{ (scene.image_prompt ?? '').length }} chars</span
                    >
                  </div>
                  <textarea
                    class="ink-input"
                  ...
                  @if (safetyWarnings(scene.image_prompt ?? '').length) {
                    <div style="margin-top:6px;display:flex;flex-direction:column;gap:4px;">
                      @for (w of safetyWarnings(scene.image_prompt ?? ''); track w.term) {
                        <div
                          style="display:flex;align-items:center;gap:8px;padding:5px 8px;background:rgba(255,140,0,.08);border:1px solid rgba(255,140,0,.25);border-radius:5px;"
                        >
                          <span style="font-size:10px;color:var(--ink-alert);flex:1;"
                            >⚠ "<code style="font-family:'JetBrains Mono',monospace;">{{
                              w.term
                            }}</code
                            >" may be flagged by Higgsfield</span
                          >
                          <button
                            class="btn-ink"
                            style="height:20px;font-size:9px;padding:0 8px;flex-shrink:0;"
                            (click)="applyFix(idx, w.term, w.fix)"
                          >
                            → {{ w.fix }}
                          </button>
                        </div>
                ...
                <div>
                  ...
                  <div style="display:flex;flex-direction:column;gap:8px;">
                    @for (field of VIDEO_PROMPT_FIELDS; track field.key) {
                      <div
                        style="display:grid;grid-template-columns:108px 1fr;gap:8px;align-items:start;"
                      >
                        <div style="padding-top:6px;">
                          <span
                            style="font-size:11px;font-weight:600;color:var(--ink-text-2);display:block;"
                            >{{ field.label }}</span
                          >
                          <span
                            style="font-size:9px;color:var(--ink-text-3);line-height:1.3;display:block;margin-top:2px;"
                            >{{ field.hint }}</span
                          >
                        </div>
                        <textarea
                    ...
                    <div
                      style="display:grid;grid-template-columns:108px 1fr;gap:8px;align-items:center;"
                    >
                      <div>
                        <span
                          style="font-size:11px;font-weight:600;color:var(--ink-text-2);display:block;"
                          >Duration</span
                        >
                        <span style="font-size:9px;color:var(--ink-text-3);"
                          >seconds for this clip</span
                        >
                      </div>
                      <div style="display:flex;align-items:center;gap:6px;">
                        <input
                          ...
                          [value]="
                            (scene.video_prompt ?? {})['durationSec'] ?? scene.duration_sec ?? ''
                          "
                          placeholder="11"
                          min="1"
                          max="60"
              ...
              <!-- Scene save footer -->
              <div
                style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-top:1px solid var(--ink-border);background:var(--ink-surface);"
              >
                <div>
                  @if (isDirty(idx)) {
                    <span style="font-size:10px;color:var(--ink-alert);font-weight:600;"
                      >● unsaved changes</span
                    >
                  } @else {
                    <span style="font-size:10px;color:var(--ink-text-3);">{{
                      scene.scene_status ? 'Saved · ' + scene.scene_status : 'No changes'
                    }}</span>
                  }
                </div>
                <div style="display:flex;gap:6px;">
                  ...
                  <button
                    class="btn-brand"
                    style="height:28px;font-size:11px;padding:0 12px;"
        ...
        @if (activeTab() === 2) {
          ...
          >
            <div>
              ...
              <p style="font-size:10px;color:var(--ink-text-3);margin:0;">
                Defaults from channel · override per-reel. Saved to
                <code style="font-family:'JetBrains Mono',monospace;">gen_config</code> jsonb.
              </p>
            </div>
            @if (item.format) {
              <span class="ink-badge ib-brand" style="font-family:'JetBrains Mono',monospace;">{{
          ...
          <div>
            ...
            <select
              class="ink-select"
              style="height:30px;font-size:12px;padding:0 8px;width:100%;max-width:280px;"
              disabled
            >
              <option>Higgsfield</option>
            </select>
            <p style="font-size:9px;color:var(--ink-text-3);margin:4px 0 0;">
              fal.ai · Replicate · RunwayML — coming soon
            </p>
          </div>
              ...
              <div>
                ...
                >
                  @for (m of IMAGE_MODELS; track m.id) {
                    <option
                      [value]="m.id"
                      [selected]="(genConfigDraft().imageModel ?? chDefault('imageModel')) === m.id"
                    >
                      {{ m.label }}
                    </option>
                  }
                  ...
                  <div style="display:flex;gap:4px;">
                    @for (r of IMAGE_RES; track r) {
                      <button
                        [class]="
                          'pill ' +
                          ((genConfigDraft().imageRes ?? chDefault('imageRes')) === r
                            ? 'pill-on'
                            : '')
                        "
                        (click)="patchConfig('imageRes', r)"
                      >
                        {{ r.toUpperCase() }}
                      </button>
                    }
                  ...
                  <div style="display:flex;gap:4px;">
                    @for (a of ASPECT_RATIOS; track a) {
                      <button
                        [class]="
                          'pill ' +
                          ((genConfigDraft().aspectRatio ?? chDefault('aspectRatio')) === a
                            ? 'pill-on'
                            : '')
                        "
                        (click)="patchConfig('aspectRatio', a)"
                      >
                        {{ a }}
                      </button>
                    }
              ...
              <div style="display:flex;flex-direction:column;gap:10px;">
                  ...
                  >
                    @for (m of VIDEO_MODELS; track m.id) {
                      <option
                        [value]="m.id"
                        [selected]="
                          (genConfigDraft().videoModel ?? chDefault('videoModel')) === m.id
                        "
                        [disabled]="item.format === '21s' && !m.endImage"
                      >
                        {{ m.label
                        }}{{ item.format === '21s' && !m.endImage ? ' — no end_image ✗' : '' }}
                      </option>
                    }
                  </select>
                  @if (item.format === '21s') {
                    <p style="font-size:9px;color:var(--ink-text-3);margin:4px 0 0;">
                      21s scenario-3 chain needs a start+end model — turbo disabled.
                    </p>
                  }
                </div>
                <div>
                  ...
                  <div style="display:flex;gap:4px;flex-wrap:wrap;">
                    @for (r of VIDEO_RES; track r) {
                      <button
                        [class]="
                          'pill ' +
                          ((genConfigDraft().videoRes ?? chDefault('videoRes')) === r
                            ? 'pill-on'
                            : '')
                        "
                        (click)="patchConfig('videoRes', r)"
                      >
                        {{ r }}
                      </button>
                    }
                ...
                @if (item.format !== '21s') {
                  <div>
                    <label
                      style="font-size:9px;color:var(--ink-text-3);text-transform:uppercase;letter-spacing:.06em;display:block;margin-bottom:4px;"
                      >Clip duration (s)</label
                    >
                    <input
                      type="number"
                      class="ink-select"
                      style="height:30px;font-size:12px;padding:0 8px;width:120px;"
                    ...
                    />
                    <p style="font-size:9px;color:var(--ink-text-3);margin:4px 0 0;">
                      {{ genConfigDraft().videoModel ?? chDefault('videoModel') }} supports
                      {{ durationRange().min }}–{{ durationRange().max }}s
                      @if (durationRange().fixed) {
                        · valid: {{ durationRange().fixed?.join(' / ') }}s
                      }
                      . Default {{ formatDurationDefault() }}s from {{ item.format }} preset.
                    </p>
                  </div>
                }
              </div>
          ...
          >
            <button
              class="btn-brand"
              [disabled]="!genConfigDirty() || saving()"
              (click)="saveConfig()"
            >
              Save config
            </button>
            ...
            <div>
              ...
              >
                ...
                <button
                  class="btn-ink"
                  style="height:26px;padding:0 10px;font-size:11px;"
                  (click)="copy(item.seo!.description)"
                >
                  Copy
                </button>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <button
                    class="btn-ink"
                    style="height:26px;padding:0 10px;font-size:11px;"
                    (click)="copy(item.seo!.hashtags.join(' '))"
                  >
                    Copy all
                  </button>
                </div>
                <div style="display:flex;gap:4px;flex-wrap:wrap;">
                  @for (tag of item.seo!.hashtags; track tag) {
                    <span class="ink-badge ib-brand" style="cursor:pointer;" (click)="copy(tag)">{{
                      tag
                    }}</span>
                  }
                </div>
            ...
            <div>
              ...
              >
                ...
                <button
                  class="btn-ink"
                  style="height:26px;padding:0 10px;font-size:11px;"
                  (click)="copy(item.rendered_video_url!)"
                >
                  Copy URL
                </button>
              </div>
              <video
                ...
                controls
                preload="metadata"
                style="width:100%;max-height:300px;border-radius:8px;background:#000;display:block;"
              ></video>
            </div>
            <div style="border-top:1px solid var(--ink-border);"></div>
          }
              ...
              <div style="display:flex;flex-direction:column;gap:6px;">
                @for (scene of item.scenes!; track scene.scene_num) {
                  @if (scene.higgsfield_image_job || scene.higgsfield_video_job) {
                    <div
                      style="background:var(--ink-raised);border:1px solid var(--ink-border);border-radius:6px;padding:8px 10px;display:flex;flex-direction:column;gap:4px;"
                    >
                      <span style="font-size:10px;font-weight:700;color:var(--ink-text-3);"
                        >Scene {{ scene.scene_num }}</span
                      >
                      @if (scene.higgsfield_image_job) {
                        <div style="display:flex;align-items:center;gap:6px;">
                          <span
                            style="font-size:10px;color:var(--ink-text-3);width:32px;flex-shrink:0;"
                            >img</span
                          >
                          <code
                            style="font-size:10px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                            >{{ scene.higgsfield_image_job }}</code
                          >
                          <button
                            class="btn-ghost-icon"
                            style="width:24px;height:24px;font-size:11px;"
                            (click)="copy(scene.higgsfield_image_job!)"
                          >
                            ⧉
                          </button>
                        </div>
                      ...
                      @if (scene.higgsfield_video_job) {
                        <div style="display:flex;align-items:center;gap:6px;">
                          ...
                          <code
                            style="font-size:10px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                            >{{ scene.higgsfield_video_job }}</code
                          >
                          <button
                            class="btn-ghost-icon"
                            style="width:24px;height:24px;font-size:11px;"
                            (click)="copy(scene.higgsfield_video_job!)"
                          >
                            ⧉
                          </button>
                        </div>
            ...
            <div>
              ...
              <div style="display:flex;flex-direction:column;gap:6px;">
                @for (p of platformRows(); track p.key) {
                  @if (p.targeted) {
                    <div
                      style="display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--ink-raised);border-radius:6px;border:1px solid var(--ink-border);"
                    >
                      <span
                        [style.width]="'8px'"
                        [style.height]="'8px'"
                      ...
                      ></span>
                      <span
                        style="font-size:12px;font-weight:600;color:var(--ink-text);width:28px;flex-shrink:0;"
                        >{{ p.label }}</span
                      >
                      <span [class]="'ink-badge ' + p.badgeClass" style="font-size:10px;">{{
                        p.status
                      }}</span>
                    </div>
                  }
      ...
      <!-- Footer -->
      <div
        style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 16px;border-top:1px solid var(--ink-border);flex-shrink:0;flex-wrap:wrap;"
      >
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
          @if (item.status === 'rendered') {
            <button
              class="btn-brand"
              style="background:var(--ink-trending);"
              (click)="markPosted()"
            >
              ✓ Mark as Posted
            </button>
          ...
          @if (item.rendered_video_url) {
            ...
            <button
              ...
              disabled
              title="Coming soon — server-side FFmpeg overlay"
            >
              💧 Add Watermark
            </button>
          }
  ...
  styles: [
    `
      ...
      .pill:hover {
        border-color: var(--ink-text-3);
      }
      .pill-on {
        background: var(--ink-brand, #4f46e5);
        border-color: var(--ink-brand, #4f46e5);
        color: #fff;
      }
    `,
  ],
...
export class ReelDetailComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private initDrafts() {
    ...
    if (this.item.channel_key) {
      this.supabase
        .getChannelConfig(this.item.channel_key)
  ...
  resolvedDuration(): number {
    return (
      this.genConfigDraft().duration ??
      this.channelConfig().duration ??
      this.formatDurationDefault()
    );
  }
  setDuration(raw: any) {
  ...
  isDirty(idx: number) {
    return this._dirty().has(idx);
  }
  private markDirty(idx: number) {
    this._dirty.update((s) => new Set([...s, idx]));
  }
  private clearDirty(idx: number) {
    this._dirty.update((s) => {
      const n = new Set(s);
      n.delete(idx);
      return n;
    });
  ...
  patchImagePrompt(idx: number, val: string) {
    this.patchScene(idx, 'image_prompt', val);
  }
  ...
  async expandBrief() {
    ...
    try {
      ...
      this.showToast(
        `${result.scenes.length} scene prompt${result.scenes.length !== 1 ? 's' : ''} generated`,
      );
    } catch (err: any) {
      this.showToast(err.message, false);
    } finally {
  ...
  visionBadgeClass(s: string): string {
    return (
      ({ pass: 'ib-standard', blocked: 'ib-breaking', retry: 'ib-alert' } as any)[s] ?? 'ib-ai'
    );
  }

  sceneStatusClass(s: string): string {
    return (
      ({ image_done: 'ib-ai', video_done: 'ib-trending', blocked: 'ib-breaking' } as any)[s] ?? ''
    );
  }

  hasJobIds() {
    return !!this.item.scenes?.some((s) => s.higgsfield_image_job || s.higgsfield_video_job);
  }
  hasPlatformActivity() {
    return ['fb', 'ig', 'yt', 'tt'].some((p) => this.item.target_platforms?.includes(p));
  }
