...
const PAGES = [
  ...
  },
  {
    ...
    icon: '🌿',
    label: 'NaturePulse',
    channels: ['wildlife/factual/EN', 'wildlife/listicle/EN'],
    accent: '#2d9b5c',
  },
  {
    key: 'NatureFrame',
    icon: '🎬',
    label: 'NatureFrame',
    channels: ['wildlife/cinematic/EN', 'wildlife/silent/EN'],
    accent: '#7c3aed',
  },
  {
    key: "France Aujourd'hui",
    icon: '🇫🇷',
    label: "France Aujourd'hui",
    channels: ['culture/silent/FR', 'culture/silent/FR-long'],
    accent: '#0055a4',
  },
  {
    key: 'Vivere in Italia',
    icon: '🇮🇹',
    label: 'Vivere in Italia',
    channels: ['culture/silent/IT', 'culture/silent/IT-long'],
    accent: '#009246',
  },
];
  ...
  template: `
    ...
    >
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <span
          style="font-family:'Playfair Display',serif;font-weight:900;font-size:18px;letter-spacing:.06em;color:var(--ink-text);"
          >SIGNAL</span
        >
        <span
          class="hidden sm:inline"
          style="font-size:10px;color:var(--ink-text-3);letter-spacing:.12em;text-transform:uppercase;"
          >Console</span
        >
        <a
          routerLink="/articles"
          style="font-size:11px;font-weight:600;color:var(--ink-text-2);text-decoration:none;padding:3px 8px;border-radius:4px;background:var(--ink-raised);letter-spacing:.05em;"
          >Articles</a
        >
        <a
          routerLink="/reels"
        ...
        >
        <a
          routerLink="/on-this-day"
          style="font-size:11px;font-weight:600;color:#b47828;text-decoration:none;padding:3px 8px;border-radius:4px;background:rgba(180,120,40,0.12);letter-spacing:.05em;"
          >📅 On This Day</a
        >
        <a
          routerLink="/longform"
      ...
      <div style="display:flex;align-items:center;gap:8px;">
        <span
          class="hidden sm:inline"
          style="font-size:11px;color:var(--ink-text-3);max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
          >{{ userEmail() }}</span
        >
        <button
          class="btn-theme"
    ...
    <div
      class="channel-strip"
      style="padding:8px 12px;border-bottom:1px solid var(--ink-border);background:var(--ink-navbar);position:sticky;top:0;z-index:10;backdrop-filter:blur(12px);"
    >
      ...
      @if (filterPage()) {
        <button
          class="channel-pill"
          (click)="openChannelConfig()"
          title="Edit generation defaults for this page (channel_configs)"
        >
          ⚙ Defaults
        </button>
      }
      ...
      <div style="max-width:960px;margin:0 auto;padding:16px 12px 64px;">
        ...
        @if (stats()) {
          <div
            ...
            style="display:flex;gap:8px;overflow-x:auto;padding-bottom:4px;margin-bottom:16px;"
          >
            <button
              class="stat-card"
            ...
            >
              <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-text);">
                {{ stats()!.total }}
              </div>
              <div
                style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
              >
                Total
              </div>
            </button>
            ...
            >
              <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-text-2);">
                {{ stats()!.pending }}
              </div>
              <div
                style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
            ...
            <button
              class="stat-card"
              [class.active]="filterStatus() === 'rendered'"
              (click)="filterByStatus('rendered')"
            >
              <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-trending);">
                {{ stats()!.rendered }}
              </div>
              <div
                style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
            ...
            <button
              class="stat-card"
              [class.active]="filterStatus() === 'posted'"
              (click)="filterByStatus('posted')"
            >
              <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-standard);">
                {{ stats()!.posted }}
              </div>
              <div
                style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
            ...
            >
              <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-breaking);">
                {{ stats()!.failed }}
              </div>
              <div
                style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
            ...
            @if ((renderQueueStats()?.queued ?? 0) + (renderQueueStats()?.processing ?? 0) > 0) {
              <button class="stat-card" style="cursor:default;border-color:var(--ink-alert);">
                <div style="font-size:26px;font-weight:700;line-height:1;color:var(--ink-alert);">
                  {{ (renderQueueStats()?.queued ?? 0) + (renderQueueStats()?.processing ?? 0) }}
                </div>
                <div
                  style="font-size:10px;letter-spacing:.07em;text-transform:uppercase;color:var(--ink-text-2);margin-top:5px;"
                >
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;align-items:center;">
          <div style="position:relative;flex:1;min-width:140px;">
            <svg
              style="position:absolute;left:10px;top:50%;transform:translateY(-50%);width:13px;height:13px;color:var(--ink-text-3);pointer-events:none;"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
          ...
          <select
            ...
            [value]="filterNiche()"
            (change)="filterNiche.set($any($event.target).value)"
          >
            <option value="">All Niches</option>
            @for (n of niches(); track n) {
          ...
          <select
            ...
            style="min-width:110px;"
            [value]="filterStyle()"
            (change)="filterStyle.set($any($event.target).value)"
          >
          ...
          <select
            ...
            [value]="filterChannel()"
            (change)="filterChannel.set($any($event.target).value); currentPage.set(0)"
          >
          ...
          <button
            class="btn-brand"
            style="height:36px;gap:5px;flex-shrink:0;position:relative;"
            (click)="showQueueDialog.set(true)"
          >
            <svg style="width:13px;height:13px;" viewBox="0 0 20 20" fill="currentColor">
              <path
                fill-rule="evenodd"
            ...
            @if ((renderQueueStats()?.queued ?? 0) + (renderQueueStats()?.processing ?? 0) > 0) {
              <span
                style="position:absolute;top:-5px;right:-5px;background:var(--ink-alert);color:white;font-size:9px;font-weight:700;border-radius:50%;width:16px;height:16px;display:flex;align-items:center;justify-content:center;"
                >{{
                  (renderQueueStats()?.queued ?? 0) + (renderQueueStats()?.processing ?? 0)
                }}</span
              >
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;">
          <span
            style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);"
            >Sort:</span
          >
          <button
            ...
            [style.background]="
              sortField() === 'created_at' ? 'rgba(99,102,241,.15)' : 'transparent'
            "
          ...
          <button
            style="padding:3px 8px;border-radius:4px;cursor:pointer;border:none;font-size:11px;font-family:'Outfit',sans-serif;font-weight:500;transition:all .15s;"
            [style.background]="
              sortField() === 'duration_sec' ? 'rgba(99,102,241,.15)' : 'transparent'
            "
            [style.color]="
              sortField() === 'duration_sec' ? 'var(--ink-brand)' : 'var(--ink-text-2)'
            "
            (click)="setSort('duration_sec')"
          >
            Duration {{ sortIcon('duration_sec') }}
          </button>
          <span
            style="margin-left:auto;font-size:11px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-3);"
            >{{ filteredItems().length }} reel{{ filteredItems().length !== 1 ? 's' : '' }}</span
          >
        </div>
        ...
        @if (selectedIds().length > 0) {
          <div class="batch-bar">
            <span style="font-size:13px;font-weight:600;color:var(--ink-brand);"
              >{{ selectedIds().length }} selected</span
            >
            <button
              class="btn-brand"
              style="background:var(--ink-trending);"
              (click)="markSelectedPosted()"
            >
              ✓ Mark Posted
            </button>
            <button class="btn-reject" (click)="deleteSelected()">Delete</button>
            <button class="btn-ink" style="margin-left:auto;" (click)="selectedIds.set([])">
              ✕ Clear
            </button>
          </div>
        }
        ...
        @if (loading()) {
          <div
            style="display:flex;flex-direction:column;align-items:center;gap:16px;padding:80px 20px;color:var(--ink-text-3);"
          >
            <span
              class="loading loading-spinner"
              style="color:var(--ink-brand);width:28px;height:28px;"
            ></span>
        ...
        @if (!loading()) {
          @if (filteredItems().length === 0) {
            ...
            <div
              class="hidden sm:flex ink-surface"
              style="align-items:center;gap:12px;padding:8px 16px;background:var(--ink-raised);margin-bottom:8px;border-radius:6px;"
            >
              ...
              <span
                style="width:110px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);"
                >Channel</span
              >
              <span
                style="flex:1;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);"
                >Title</span
              >
              <span
                style="width:50px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:right;"
                >Dur.</span
              >
              <span
                style="width:80px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:center;"
                >Status</span
              >
              <span
                style="width:64px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:center;"
                >Platforms</span
              >
              <span
                style="width:70px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:right;"
                >Date</span
              >
              ...
              @for (item of pagedItems(); track item.id) {
                ...
                >
                  ...
                  <div
                    ...
                    style="width:110px;flex-shrink:0;flex-direction:column;gap:3px;"
                  >
                    <span
                      class="ink-badge"
                      [style.background]="STYLE_COLORS[item.style]"
                      [style.color]="STYLE_TEXT[item.style]"
                      style="font-size:9px;text-transform:uppercase;letter-spacing:.06em;width:fit-content;"
                      >{{ item.style }}</span
                    >
                    <span
                      style="font-size:10px;color:var(--ink-text-3);font-family:'JetBrains Mono',monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"
                      [title]="item.channel_key"
                      >{{ item.niche }}</span
                    >
                  </div>
                  ...
                  <div style="flex:1;min-width:0;">
                    ...
                    <p
                      style="font-size:13px;font-weight:500;line-height:1.4;color:var(--ink-text);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;"
                    >
                      {{ item.title ?? item.channel_key }}
                    </p>
                    <div
                      style="display:flex;align-items:center;gap:6px;margin-top:4px;font-size:11px;color:var(--ink-text-2);"
                    >
                  ...
                  <div class="hidden sm:block" style="width:50px;flex-shrink:0;text-align:right;">
                    <span
                      style="font-size:12px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-2);"
                    >
                  ...
                  <div
                    class="hidden sm:flex"
                    style="width:80px;flex-shrink:0;justify-content:center;"
                  >
                    <span
                      [class]="'ink-badge ' + statusBadgeClass(item.status)"
                      style="font-size:9px;text-transform:uppercase;letter-spacing:.05em;"
                      >{{ item.status }}</span
                    >
                  </div>
                  ...
                  <div
                    class="hidden sm:flex"
                    style="width:64px;flex-shrink:0;justify-content:center;gap:5px;align-items:center;"
                  >
                    @for (
                      p of [
                        ['FB', 'fb'],
                      ...
                      <span
                        [title]="platformTitle(item, p[1])"
                        [style.width]="'10px'"
                        [style.height]="'10px'"
                        [style.border-radius]="'50%'"
                        [style.background]="platformDotColor(item, p[1])"
                      ></span>
                    }
                  </div>
                  ...
                  <div class="hidden sm:block" style="width:70px;flex-shrink:0;text-align:right;">
                    <span
                      ...
                      >{{ item.created_at | date: 'dd MMM' }}</span
                    >
                  </div>

                  <!-- Actions -->
                  <div
                    style="flex-shrink:0;display:flex;flex-direction:column;gap:4px;"
                    (click)="$event.stopPropagation()"
                  >
                    @if (item.status === 'rendered') {
                      <button
                        class="btn-ghost-icon"
                        style="color:var(--ink-trending);"
                        title="Mark as posted"
                        (click)="markPosted(item)"
                      >
                        ✓
                      </button>
                    }
                    <button class="btn-ghost-icon danger" title="Delete" (click)="deleteOne(item)">
                      ✕
                    </button>
                  </div>
                </div>
            ...
            @if (filteredItems().length > pageSize) {
              <div
                style="display:flex;align-items:center;justify-content:center;gap:16px;padding-top:16px;"
              >
                <button
                  class="btn-ink"
                ...
                <span
                  style="font-size:12px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-2);"
                  >{{ pageStart() }}–{{ pageEnd() }} / {{ filteredItems().length }}</span
                >
                <button
                  class="btn-ink"
                  style="height:32px;padding:0 12px;font-size:12px;"
    ...
    @if (selectedItem()) {
      <div style="position:fixed;inset:0;z-index:50;" class="animate-fade-in">
        ...
        <div
          class="detail-panel"
          style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:min(700px,calc(100vw - 24px));max-height:90vh;border-radius:12px;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,.6),0 0 0 1px rgba(255,255,255,.06);"
        >
          <app-reel-detail
            [item]="selectedItem()!"
    ...
    @if (showQueueDialog()) {
      <div style="position:fixed;inset:0;z-index:50;" class="animate-fade-in">
        <div
          style="position:absolute;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(8px);"
          (click)="showQueueDialog.set(false)"
        ></div>
        <div
          style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);background:var(--ink-surface);border:1px solid var(--ink-border);border-radius:12px;width:min(480px,calc(100vw - 32px));max-height:80vh;overflow-y:auto;z-index:10;display:flex;flex-direction:column;"
        >
          <!-- Dialog header -->
          <div
            style="padding:14px 18px;border-bottom:1px solid var(--ink-border);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;"
          >
            <span style="font-size:14px;font-weight:700;color:var(--ink-text);"
              >Queue New Reel</span
            >
          ...
          <div style="padding:16px 18px;overflow-y:auto;">
            <p style="font-size:12px;color:var(--ink-text-2);margin:0 0 14px;line-height:1.6;">
              Select a channel to queue a render job. Then run
              <code
                style="background:var(--ink-raised);padding:2px 6px;border-radius:3px;font-family:'JetBrains Mono',monospace;font-size:11px;"
                >node src/scripts/process-queue.js</code
              >
              in the reels-pipeline terminal.
            </p>
            <div style="display:flex;flex-direction:column;gap:6px;">
              @for (ch of AVAILABLE_CHANNELS; track ch.key) {
                ...
                >
                  <div>
                    <div style="font-size:13px;font-weight:600;color:var(--ink-text);">
                      {{ ch.page }}
                    </div>
                    <div
                      style="font-size:10px;color:var(--ink-text-3);margin-top:2px;font-family:'JetBrains Mono',monospace;"
                    >
                      {{ ch.key }}
                    </div>
                  </div>
                  <span class="ink-badge" style="font-size:9px;flex-shrink:0;margin-left:12px;">{{
                    ch.sub
                  }}</span>
                </button>
              }
...
export class ReelListComponent implements OnInit {
  ...
  channelCounts = computed(() => {
    ...
    for (const item of this._allItems())
      counts[item.channel_key] = (counts[item.channel_key] ?? 0) + 1;
    return counts;
  });
  ...
  filteredItems = computed(() => {
    ...
    if (q)
      items = items.filter(
        (i) => (i.title ?? '').toLowerCase().includes(q) || i.channel_key.toLowerCase().includes(q),
      );
  ...
  pageEnd = computed(() =>
    Math.min((this.currentPage() + 1) * this.pageSize, this.filteredItems().length),
  );
  ...
  allSelected = computed(
    () =>
      ...
      this.pagedItems().every((i) => this.selectedIds().includes(i.id)),
  );
  ...
  openChannelConfig() {
    ...
    if (!page) return;
    const key =
      this.filterChannel() && page.channels.includes(this.filterChannel())
        ? this.filterChannel()
        : page.channels[0];
  ...
  async markSelectedPosted() {
    ...
    try {
      await Promise.all(
        ids.map((id) =>
          this.supabase.updateContentItemStatus(id, 'posted', {
            fb_status: 'posted',
            fb_posted_at: new Date().toISOString(),
          }),
        ),
  ...
  visionBadgeClass(status: string): string {
    return (
      ({ pass: 'ib-standard', blocked: 'ib-breaking', retry: 'ib-alert' } as any)[status] ?? 'ib-ai'
    );
  }
  ...
  platformRows(item: ContentItem): {
    ...
    return [
      ...
      return {
        ...
        dotColor,
        badgeClass,
        postId: targeted ? ((item as any)[idField] ?? null) : null,
      };
    });
  }
}
