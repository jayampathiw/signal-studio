...
const TRACK_COLOR: Record<string, string> = Object.fromEntries(
  TRACKS.map((t) => [t.value, t.color]),
);
...
@Component({
  ...
  template: `
    ...
    >
      ...
      @if (saveError()) {
        <div
          style="margin-bottom:12px;padding:8px 12px;border-radius:6px;background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.2);font-size:12px;color:#f87171;"
        >
          {{ saveError() }}
        </div>
      }
      @if (saveOk()) {
        <div
          style="margin-bottom:12px;padding:8px 12px;border-radius:6px;background:rgba(74,222,128,.08);border:1px solid rgba(74,222,128,.2);font-size:12px;color:#4ade80;"
        >
          Audio plan saved.
        </div>
      }
      ...
      @if (rows().length === 0) {
        <div style="text-align:center;padding:32px;color:#475569;font-size:13px;">
          ...
          <button
            (click)="addRow()"
            style="margin-top:12px;padding:6px 14px;border-radius:6px;background:#1e1e1e;color:#94a3b8;border:1px solid #262626;font-size:12px;cursor:pointer;"
          >
        ...
        <div
          style="display:flex;height:18px;border-radius:6px;overflow:hidden;margin-bottom:16px;gap:1px;"
        >
        ...
        <!-- Timeline labels -->
        <div
          style="display:flex;justify-content:space-between;font-size:10px;color:#475569;margin-bottom:16px;"
        >
          <span>0:00</span>
        ...
        <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:14px;">
          <!-- Header -->
          <div
            style="display:grid;grid-template-columns:36px 72px 72px 1fr 90px 32px;gap:8px;align-items:center;padding:0 4px;"
          >
            <div style="font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.4px;">
              Act
            </div>
            <div style="font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.4px;">
              From (s)
          ...
          @for (row of rows(); track $index; let i = $index) {
            <div
              style="display:grid;grid-template-columns:36px 72px 72px 1fr 90px 32px;gap:8px;align-items:center;padding:6px 4px;border-radius:6px;background:#0d0d0d;"
            >
              <!-- Act badge -->
              <div
                style="text-align:center;padding:2px 0;border-radius:4px;font-size:11px;font-weight:700;background:#1a1a1a;color:#64748b;"
              >
                {{ i }}
              </div>
              <!-- From -->
              ...
              >
                @for (t of tracks; track t.value) {
                  <option [value]="t.value" [selected]="t.value === row.track">
                    {{ t.label }}
                  </option>
                }
              </select>
...
export class LongformAudioComponent implements OnInit {
  ...
  patchRow(i: number, field: keyof AudioPlanSegment, value: string | number) {
    const next = this.rows().map((r, idx) => (idx === i ? { ...r, [field]: value } : r));
    this.rows.set(next);
  }
  ...
  async save() {
    ...
    try {
      ...
      const payload = raw && !Array.isArray(raw) ? { ...raw, segments: this.rows() } : this.rows();
