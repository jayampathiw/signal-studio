...
const CHANNEL_META: Record<
  string,
  { slug: string; pageName: string; logoFile: string; description: string }
> = {
    ...
    <div style="max-width:960px;margin:0 auto;padding:32px 20px;">
      <div style="margin-bottom:28px;">
        ...
        <p style="margin:4px 0 0;font-size:13px;color:#475569;">
          Select a channel to browse its videos
        </p>
      </div>
      ...
      @if (!loading()) {
        ...
        >
          @for (ch of channels(); track ch.slug) {
            <a
              [routerLink]="['/longform/channel', ch.slug]"
            ...
            >
              <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;">
                <img
                  [src]="'logos/' + ch.logoFile"
                  [alt]="ch.pageName"
                  style="width:48px;height:48px;border-radius:8px;object-fit:contain;background:#1a1a1a;padding:4px;"
                />
                <div style="font-size:16px;font-weight:700;color:#e2e8f0;line-height:1.3;">
                  {{ ch.pageName }}
                </div>
              </div>
              <div style="font-size:12px;color:#475569;margin-bottom:16px;line-height:1.5;">
                {{ ch.description }}
              </div>
