  ...
  template: `
    ...
    <div style="background:#111;border:1px solid #1a1a1a;border-radius:12px;overflow:hidden;margin-bottom:24px;">
      <div style="padding:16px 20px;border-bottom:1px solid #1a1a1a;display:flex;align-items:center;justify-content:space-between;">
        ...
        <span style="padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(37,99,235,.2);color:#60a5fa;">
          Awaiting approval
        </span>
      </div>
    ...
    @if (project.seo) {
      <div style="background:#111;border:1px solid #1a1a1a;border-radius:12px;padding:20px;margin-bottom:24px;">
        <div style="font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:.6px;margin-bottom:14px;">SEO Package</div>

        <div style="margin-bottom:12px;">
          <div style="font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Title</div>
          <div style="font-size:14px;color:#e2e8f0;font-weight:500;">{{ project.seo.title }}</div>
        </div>

        <div style="margin-bottom:12px;">
          <div style="font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;">Description</div>
          <div style="font-size:12px;color:#94a3b8;line-height:1.6;white-space:pre-wrap;max-height:120px;overflow-y:auto;">{{ project.seo.description }}</div>
        </div>

        @if (project.seo.hashtags?.length) {
          <div>
            <div style="font-size:10px;color:#475569;text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">Hashtags</div>
    ...
    <div style="background:#111;border:1px solid #1a1a1a;border-radius:12px;padding:20px;margin-bottom:24px;">
      <div style="font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:.6px;margin-bottom:14px;">Pre-publish checklist</div>
        ...
        @for (item of checklist; track $index; let i = $index) {
          <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;">
            ...
            <span style="font-size:13px;"
                  [style.color]="checked()[i] ? '#e2e8f0' : '#64748b'">
              {{ item }}
            </span>
          </label>
        }
      ...
      @if (!confirmStep()) {
        ...
        @if (!allChecked()) {
          <div style="margin-top:8px;font-size:11px;color:#475569;">Complete the checklist to enable publish.</div>
        }
      } @else {
        <!-- Confirm step -->
        <div style="padding:14px;border-radius:8px;border:1px solid rgba(234,179,8,.3);background:rgba(234,179,8,.06);">
          <div style="font-size:13px;color:#fbbf24;font-weight:600;margin-bottom:4px;">Publish to all platforms?</div>
