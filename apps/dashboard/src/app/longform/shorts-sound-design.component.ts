    ...
    >
      ...
      >
        ...
        <span style="font-size:11px;color:#475569;"
          >read-only — derived from the shot list's music_bed / heartbeat_layer columns</span
        >
      </div>

      @if (!bed().length && !heartbeat().length) {
        <div style="text-align:center;padding:32px;color:#475569;font-size:13px;">
          No sound design data — this shot list may not have set
          <code style="background:#1a1a1a;padding:1px 5px;border-radius:3px;">music_bed</code>/<code
            style="background:#1a1a1a;padding:1px 5px;border-radius:3px;"
            >heartbeat_layer</code
          >, or predates the v3 template.
        </div>
      } @else {
        @if (bed().length) {
          <div style="margin-bottom:16px;">
            <div
              style="font-size:11px;color:#475569;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;"
            >
              Music bed
            </div>
            <div style="display:flex;flex-direction:column;gap:6px;">
              @for (seg of bed(); track $index) {
                <div
                  style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:#0d0d0d;border-radius:6px;border:1px solid #1e1e1e;"
                >
                  <span style="font-family:monospace;font-size:11px;color:#64748b;min-width:90px;"
                    >S{{ seg.start_scene
                    }}{{ seg.start_scene !== seg.end_scene ? '–' + seg.end_scene : '' }}</span
                  >
...
export class ShortsSoundDesignComponent {
  ...
  @Input() set project(p: LongformProject) {
    this._project.set(p);
  }

  private soundDesign = computed(() => {
    const meta = this._project()?.shot_list_meta as {
      sound_design?: { bed?: BedSegment[]; heartbeat?: HeartbeatSpan[] };
    } | null;
