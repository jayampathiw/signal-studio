  ...
  template: `
    ...
    >
      ...
      >
        ...
        >
          <span
            style="font-size:12px;color:#475569;transition:transform .2s;"
        ...
        <div style="display:flex;align-items:center;gap:12px;">
          <span style="font-size:12px;color:#475569;"
            >{{ uploadedCount() }} / {{ totalSlots() }} uploaded</span
          >
          <!-- View toggle — only shown when expanded and browsing the grid/list -->
          @if (
            !collapsed() && (project.status !== 'storyboard' || uploadMethod() === 'individual')
          ) {
            <div style="display:flex;border:1px solid #262626;border-radius:6px;overflow:hidden;">
              <button
                ...
                [style.color]="!listView() ? 'white' : '#64748b'"
              >
                Grid
              </button>
              <button
      ...
      @if (!loading() && totalSlots() > 0) {
        <div
          style="background:#1a1a1a;border-radius:4px;height:4px;overflow:hidden;margin-bottom:20px;"
        >
          <div
            style="height:100%;background:#4ade80;transition:width .3s;"
      ...
      @if (!collapsed()) {
        @if (loading()) {
          ...
          >
            <button
              (click)="uploadMethod.set('individual')"
            ...
            >
              <div style="font-size:13px;font-weight:600;color:#94a3b8;margin-bottom:4px;">
                🖼️ Upload Still Images
              </div>
              <div style="font-size:11px;color:#475569;line-height:1.5;">
                Browse scene-by-scene and click each slot to upload its image one at a time.
              </div>
            </button>
            <button
              (click)="uploadMethod.set('filename')"
            ...
            >
              <div style="font-size:13px;font-weight:600;color:#4ade80;margin-bottom:4px;">
                📁 Bulk Upload by Filename
              </div>
              <div style="font-size:11px;color:#475569;line-height:1.5;">
                Files named
                <code style="background:#111;padding:1px 4px;border-radius:3px;">S7-A.jpeg</code> —
                slot assigned instantly from the filename, no AI needed.
              </div>
            </button>
            <button
              (click)="uploadMethod.set('automatch')"
            ...
            >
              <div style="font-size:13px;font-weight:600;color:#60a5fa;margin-bottom:4px;">
                ⚡ Auto-Match via Claude Vision
              </div>
              <div style="font-size:11px;color:#475569;line-height:1.5;">
                Any filename — Claude Vision looks at each image and assigns it to the correct slot.
              </div>
            </button>
          </div>
        } @else {
            ...
            @if (listView()) {
                ...
                @for (scene of scenes(); track scene.scene_n) {
                  @for (slot of scene.slots; track slot.cut) {
                    @if (isEditorBuild(slot)) {
                      ...
                      <div
                        style="display:grid;grid-template-columns:64px 80px 1fr 110px;gap:12px;align-items:center;padding:10px 12px;border-radius:8px;border:1px solid #131313;background:#0a0a0a;opacity:.45;"
                      >
                        <div
                          style="aspect-ratio:16/9;border-radius:5px;background:#111;display:flex;align-items:center;justify-content:center;font-size:18px;"
                        >
                          🎬
                        </div>
                        <div>
                          <div
                            style="font-size:13px;font-weight:700;color:#475569;font-family:monospace;"
                          >
                            S{{ slot.scene_n }}-{{ slot.cut }}
                          </div>
                          <span
                            style="font-size:9px;padding:1px 5px;border-radius:3px;font-weight:600;background:rgba(100,116,139,.2);color:#64748b;"
                            >editor build</span
                          >
                        </div>
                        <div style="font-size:11px;color:#334155;font-style:italic;">
                          No image needed — built in editor
                        </div>
                        <div style="font-size:11px;color:#334155;text-align:right;">—</div>
                      </div>
                    } @else {
                      <div
                        style="display:grid;grid-template-columns:64px 80px 1fr 110px;gap:12px;align-items:center;padding:10px 12px;border-radius:8px;border:1px solid;"
                        [style.border-color]="
                          slot.uploading ? '#3b82f6' : slot.still?.clip_url ? '#1e293b' : '#1a1a1a'
                        "
                        [style.background]="slot.still?.clip_url ? 'rgba(30,41,59,.3)' : '#0d0d0d'"
                      >
                        <!-- Thumbnail -->
                        <div
                          style="position:relative;aspect-ratio:16/9;border-radius:5px;overflow:hidden;background:#1a1a1a;cursor:pointer;"
                          (click)="openPicker(slot)"
                        >
                          @if (slot.still?.clip_url) {
                            <img
                              [src]="slot.still!.clip_url!"
                            ...
                            <div
                              style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#334155;font-size:18px;"
                            >
                              +
                            </div>
                          }
                        ...
                        <div>
                          <div
                            style="font-size:13px;font-weight:700;color:#94a3b8;font-family:monospace;"
                          >
                            S{{ slot.scene_n }}-{{ slot.cut }}
                          </div>
                          @if (slot.still) {
                            <span
                              style="font-size:9px;padding:1px 5px;border-radius:3px;font-weight:600;"
                        ...
                        <!-- Prompt text -->
                        <div
                          style="font-size:11px;color:#475569;line-height:1.5;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;"
                        >
                          {{ slot.still?.prompt ?? '—' }}
                        </div>
                        ...
                        @if (slot.error) {
                          <div
                            style="grid-column:1/-1;font-size:11px;color:#f87171;padding-left:4px;"
                          >
                            {{ slot.error }}
                          </div>
                        }
              ...
              @for (scene of scenes(); track scene.scene_n) {
                <div style="margin-bottom:24px;">
                  <div
                    style="font-size:12px;font-weight:600;color:#64748b;text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;"
                  >
                    Scene {{ scene.scene_n }}
                  </div>
                  <div
                  ...
                  >
                    @for (slot of scene.slots; track slot.cut) {
                      @if (isEditorBuild(slot)) {
                        ...
                        <div
                          style="border-radius:8px;overflow:hidden;border:1px solid #131313;background:#0a0a0a;opacity:.4;"
                        >
                          <div
                            style="position:relative;aspect-ratio:16/9;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;"
                          >
                          ...
                          <div
                            style="padding:6px 8px;font-size:10px;color:#334155;font-style:italic;min-height:32px;"
                          >
                        ...
                        <div
                          style="border-radius:8px;overflow:hidden;border:1px solid;background:#0d0d0d;cursor:pointer;"
                          [style.border-color]="
                            slot.uploading
                              ? '#3b82f6'
                              : slot.still?.clip_url
                        ...
                        >
                          ...
                          <div style="position:relative;aspect-ratio:16/9;">
                            @if (slot.still?.clip_url && !slot.uploading) {
                              ...
                              <div
                                style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.7);"
                              >
                                <div class="spinner"></div>
                              </div>
                            } @else {
                              <div
                                style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;"
                              >
                                <div
                                  style="width:28px;height:28px;border-radius:50%;border:1.5px dashed #2d4a3a;display:flex;align-items:center;justify-content:center;color:#2d6a4f;font-size:16px;"
                                >
                                  +
                                </div>
                            ...
                            <div
                              style="position:absolute;top:5px;left:5px;display:flex;gap:4px;align-items:center;"
                            >
                              <span
                                style="padding:1px 6px;border-radius:4px;font-size:10px;font-weight:700;background:rgba(0,0,0,.7);color:#94a3b8;"
                              >
                                S{{ slot.scene_n }}-{{ slot.cut }}
                              </span>
                              @if (slot.still) {
                                <span
                                  style="padding:1px 5px;border-radius:4px;font-size:9px;font-weight:600;"
                                  [style.background]="statusBg(slot.still.status)"
                                  [style.color]="statusColor(slot.still.status)"
                                >
                                  {{ slot.still.status }}
                          ...
                          <div
                            style="padding:6px 8px;font-size:10px;color:#475569;line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;min-height:32px;"
                          >
                            {{ slot.still?.prompt ?? '' }}
                          </div>
                        </div>
      ...
      @if (project.status === 'storyboard' && uploadMethod() === 'filename') {
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid #1a1a1a;">
          ...
          <div
            style="background:#0a1a0a;border:1px dashed #1a3a1a;border-radius:10px;padding:18px 20px;margin-bottom:12px;"
          >
            <div
            ...
            >
              <div>
                ...
                <div style="font-size:11px;color:#475569;margin-top:2px;">
                  Files must be named
                  <code style="background:#111;padding:1px 4px;border-radius:3px;">S7-A.jpeg</code>
                  — slot assigned from filename instantly, no AI needed
                </div>
              </div>
              <button
                (click)="triggerFilenameUpload()"
            ...
            @if (filenameUploading() || filenameProgress().log.length > 0) {
              @if (filenameUploading()) {
                <div style="margin-bottom:6px;">
                  <div
                    style="display:flex;justify-content:space-between;font-size:11px;color:#475569;margin-bottom:3px;"
                  >
                    <span>{{ filenameProgress().current }} of {{ filenameProgress().total }}</span>
                    <span
                      >{{
                        Math.round((filenameProgress().current / filenameProgress().total) * 100)
                      }}%</span
                    >
                  </div>
                  <div style="background:#1a1a1a;border-radius:3px;height:3px;overflow:hidden;">
                    <div
                      style="height:100%;background:#4ade80;transition:width .3s;"
                      [style.width]="
                        (filenameProgress().current / filenameProgress().total) * 100 + '%'
                      "
                    ></div>
                  </div>
                </div>
              ...
              <div
                style="max-height:140px;overflow-y:auto;display:flex;flex-direction:column;gap:2px;"
              >
                @for (line of filenameProgress().log; track $index) {
                  <div
                    style="font-size:11px;font-family:monospace;padding:2px 0;"
                    [style.color]="
                      line.startsWith('✓')
                        ? '#4ade80'
                        : line.startsWith('⚠')
          ...
          <div style="font-size:12px;color:#475569;">
            ...
            @if (uploadedCount() < totalSlots()) {
              · {{ totalSlots() - uploadedCount() }} remaining
            }
          </div>
        </div>
      }
      ...
      @if (project.status === 'storyboard' && uploadMethod() === 'automatch') {
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid #1a1a1a;">
          ...
          <div
            style="background:#0a1628;border:1px dashed #1e3a5f;border-radius:10px;padding:18px 20px;margin-bottom:16px;"
          >
            <div
            ...
            >
              <div>
                ...
                <div style="font-size:11px;color:#475569;margin-top:2px;">
                  Any filename — Claude Vision analyses each image and assigns it to the correct
                  slot
                </div>
              </div>
              <button
            ...
            @if (autoMatching() || matchProgress().log.length > 0) {
              @if (autoMatching()) {
                <div style="margin-bottom:8px;">
                  <div
                    style="display:flex;justify-content:space-between;font-size:11px;color:#475569;margin-bottom:4px;"
                  >
                    <span
                      >Processing {{ matchProgress().current }} of {{ matchProgress().total }}</span
                    >
                    <span
                      >{{
                  ...
                  <div style="background:#1a1a1a;border-radius:3px;height:3px;overflow:hidden;">
                    <div
                      style="height:100%;background:#3b82f6;transition:width .3s;"
...
export class LongformWorkbenchComponent implements OnInit {
  ...
  matchProgress = signal<{ current: number; total: number; log: string[] }>({
    current: 0,
    total: 0,
    log: [],
  });
  filenameUploading = signal(false);
  filenameProgress = signal<{ current: number; total: number; log: string[] }>({
    current: 0,
    total: 0,
    log: [],
  });
  ...
  totalSlots = computed(() =>
    this.scenes().reduce(
      (n, s) => n + s.slots.filter((slot) => !this.isEditorBuild(slot)).length,
      0,
    ),
  );
  ...
  async onFilenameUpload(event: Event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      this.filenameProgress.set({
        ...p,
        current: i + 1,
        log: [...p.log, `Uploading ${file.name}…`],
      });
      ...
      if (!slotM) {
        ...
        this.filenameProgress.set({
          ...updated,
          log: [
            ...updated.log.slice(0, -1),
            `⚠ ${file.name} — skipped (filename must be S{N}-{CUT}.jpeg)`,
          ],
        });
        continue;
      }
      ...
      try {
        ...
        this.filenameProgress.set({
          ...updated,
          log: [...updated.log.slice(0, -1), `✓ ${file.name} → S${scene_n}-${cut}`],
        });
      } catch (e: any) {
        const updated = this.filenameProgress();
        this.filenameProgress.set({
          ...updated,
          log: [...updated.log.slice(0, -1), `✗ ${file.name}: ${e.message}`],
        });
      }
    }
  ...
  async onAutoMatchFiles(event: Event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const msg = isNoMatch
          ? `${file.name} — skipped (no scene match)`
          : isFilled
            ? `${file.name} — slot already filled, skipped`
            : `${file.name}: ${msg_str}`;
  ...
  statusBg(status: string): string {
    const map: Record<string, string> = {
      passed: 'rgba(22,163,74,.3)',
      generated: 'rgba(37,99,235,.3)',
      failed: 'rgba(220,38,38,.3)',
      blocked: 'rgba(220,38,38,.3)',
      validating: 'rgba(234,179,8,.3)',
      generating: 'rgba(37,99,235,.3)',
