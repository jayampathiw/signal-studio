      ...
      >
        ...
        <div style="display:flex;align-items:center;gap:8px;">
          <a
            routerLink="/longform"
            class="btn-ink"
            style="height:30px;font-size:12px;padding:0 12px;text-decoration:none;"
            >🎬 Video Channels</a
          >
          <a
            routerLink="/articles"
      ...
      <div style="max-width:760px;margin:0 auto;padding:24px 16px 64px;">
        ...
        @if (step() === 1) {
          <div style="margin-bottom:24px;">
            <h1 style="font-size:20px;font-weight:700;color:var(--ink-text);margin-bottom:4px;">
              Upload Research
            </h1>
            <p style="font-size:13px;color:var(--ink-text-2);line-height:1.5;">
              Paste URLs, article excerpts, tweet threads, CSV rows, or any research notes. Claude
              will extract and analyse each article.
            </p>
          </div>
          ...
          <div style="margin-bottom:16px;">
            ...
            <div style="display:flex;gap:8px;">
              @for (c of countries; track c.code) {
                <button
                  [style.background]="
                    selectedCountry() === c.code ? 'var(--ink-brand)' : 'var(--ink-raised)'
                  "
          ...
          <div style="display:flex;gap:8px;margin-bottom:24px;">
            ...
            @if (fileName()) {
              <span style="font-size:11px;color:var(--ink-standard);align-self:center;"
                >✓ {{ fileName() }}</span
              >
            }
          </div>
          ...
          @if (error()) {
            <p style="font-size:12px;color:var(--ink-breaking);margin-bottom:12px;">
              {{ error() }}
            </p>
          }
          ...
          <button
            class="btn-brand"
            style="width:100%;justify-content:center;height:44px;font-size:14px;font-weight:700;"
        ...
        @if (step() === 2) {
          <div
            style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:320px;gap:20px;"
          >
            <span
              class="loading loading-spinner"
              style="width:36px;height:36px;color:var(--ink-brand);"
            ></span>
            <div style="text-align:center;">
              <p style="font-size:15px;font-weight:600;color:var(--ink-text);margin-bottom:4px;">
                Analysing with Claude…
              </p>
              <p style="font-size:12px;color:var(--ink-text-3);">
                Extracting articles and running policy checks
              </p>
            </div>
          </div>
        }
        ...
        @if (step() === 3) {
          <div
            style="margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;"
          >
            <div>
              <h1 style="font-size:18px;font-weight:700;color:var(--ink-text);margin-bottom:2px;">
                {{ analyzedArticles().length }} article{{
                  analyzedArticles().length !== 1 ? 's' : ''
                }}
                found
              </h1>
              <p style="font-size:12px;color:var(--ink-text-3);">
                Review, edit, and approve each article before saving to the pipeline.
              </p>
            </div>
            <div style="display:flex;gap:8px;align-items:center;">
              <span style="font-size:12px;color:var(--ink-text-3);"
                >{{ savedCount() }} saved · {{ skippedCount() }} skipped</span
              >
              @if (allActioned()) {
                <a
                  routerLink="/articles"
                  class="btn-brand"
                  style="text-decoration:none;height:32px;padding:0 14px;font-size:12px;"
                >
                  View in Articles →
                </a>
              }
          ...
          @for (article of analyzedArticles(); track $index; let i = $index) {
            <div
              [style.opacity]="savedIndices().has(i) || skippedIndices().has(i) ? '.45' : '1'"
              style="background:var(--ink-surface);border:1px solid var(--ink-border);border-radius:10px;margin-bottom:16px;overflow:hidden;"
            >
              <!-- Card header -->
              <div
                style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid var(--ink-border);background:var(--ink-raised);"
              >
                <div style="display:flex;align-items:center;gap:8px;">
                  <span
                    style="font-size:11px;color:var(--ink-text-3);font-weight:600;letter-spacing:.06em;"
                    >#{{ i + 1 }}</span
                  >
                ...
                @if (savedIndices().has(i)) {
                  <span style="font-size:11px;color:var(--ink-standard);font-weight:600;"
                    >✓ Saved</span
                  >
                } @else if (skippedIndices().has(i)) {
                  <span style="font-size:11px;color:var(--ink-text-3);font-weight:600;"
                    >✕ Skipped</span
                  >
                }
              </div>
              ...
              <div style="padding:14px;display:flex;flex-direction:column;gap:12px;">
                ...
                @if (article.policy_flags.length > 0) {
                  <div
                    style="background:rgba(255,54,54,.08);border:1px solid rgba(255,54,54,.25);border-radius:6px;padding:10px 12px;"
                  >
                    <p
                      style="font-size:11px;font-weight:700;color:var(--ink-breaking);letter-spacing:.06em;text-transform:uppercase;margin-bottom:6px;"
                    >
                      ⚠️ Policy flags
                    </p>
                    @for (flag of article.policy_flags; track flag) {
                      <p style="font-size:12px;color:var(--ink-breaking);margin-bottom:2px;">
                        • {{ flag }}
                      </p>
                    }
                  </div>
                ...
                <div>
                  ...
                  <input
                    class="ink-input"
                    type="text"
                    style="width:100%;box-sizing:border-box;font-size:14px;font-weight:600;"
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                  <div>
                    ...
                    <input
                      class="ink-input"
                      type="text"
                      style="width:100%;box-sizing:border-box;font-size:11px;font-family:'JetBrains Mono',monospace;"
                ...
                <div>
                  ...
                  <textarea
                    class="ink-input"
                    rows="4"
                    style="width:100%;resize:vertical;font-size:13px;box-sizing:border-box;line-height:1.55;"
                    [value]="editedData[i].summary"
                    (input)="editedData[i].summary = $any($event.target).value"
                  ></textarea>
                </div>
                ...
                @if (article.policy_flags.length > 0) {
                  <div>
                    ...
                    <textarea
                      class="ink-input"
                      rows="3"
                      style="width:100%;resize:vertical;font-size:13px;box-sizing:border-box;line-height:1.55;border-color:rgba(255,140,0,.4);"
                      [value]="editedData[i].suggested_angle"
                      (input)="editedData[i].suggested_angle = $any($event.target).value"
                    ></textarea>
                  </div>
                }
                ...
                <div>
                  ...
                  <textarea
                    class="ink-input"
                    rows="2"
                    style="width:100%;resize:vertical;font-size:12px;box-sizing:border-box;"
                    placeholder="Add research context, angle decisions, posting notes…"
                    [value]="editedData[i].editor_notes"
                    (input)="editedData[i].editor_notes = $any($event.target).value"
                  ></textarea>
                </div>
                ...
                @if (!savedIndices().has(i) && !skippedIndices().has(i)) {
                  <div
                    style="display:flex;gap:8px;padding-top:4px;border-top:1px solid var(--ink-border);"
                  >
                    <button
                      class="btn-brand"
          ...
          @if (allActioned() && analyzedArticles().length > 0) {
            <div
              style="background:var(--ink-surface);border:1px solid var(--ink-border);border-radius:10px;padding:20px;text-align:center;margin-top:8px;"
            >
              ...
              <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;">
                <a routerLink="/articles" class="btn-brand" style="text-decoration:none;"
                  >View in Articles →</a
                >
...
export class UploadComponent {
  ...
  savedCount() {
    return this.savedIndices().size;
  }
  skippedCount() {
    return this.skippedIndices().size;
  }
  allActioned() {
