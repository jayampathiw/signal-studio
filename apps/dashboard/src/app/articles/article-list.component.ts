  ...
  template: `
    ...
    >
      <div style="display:flex;align-items:center;gap:10px;">
        ...
        <a
          routerLink="/upload"
          style="font-size:11px;font-weight:600;color:var(--ink-standard);text-decoration:none;padding:3px 8px;border-radius:4px;background:rgba(0,204,112,.1);letter-spacing:.05em;"
          >⬆ Upload</a
        >
        <a
          routerLink="/reels"
      ...
      <div style="max-width:900px;margin:0 auto;padding:16px 12px 64px;">
        ...
        @if (stats()) {
          ...
          >
            ...
            <button
              class="stat-card"
              [class.active]="filterStatus() === 'manual_review'"
              (click)="filterByStatus('manual_review')"
            >
        ...
        @if (filterCountry()) {
          <div style="margin-bottom:14px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <button
              class="btn-brand"
              style="height:34px;font-size:12px;padding:0 14px;gap:6px;"
              (click)="toggleTopPicks()"
            >
              🏆 {{ topPicksOnly() ? 'Showing' : 'Show' }} top articles for
              {{ countryName(filterCountry()) }}
            </button>
            @if (topPicksOnly()) {
              <span
                style="font-size:11px;font-family:'JetBrains Mono',monospace;color:var(--ink-text-3);"
                >top {{ topPicksCount }} by score</span
              >
              <button
                class="ink-badge"
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:8px;align-items:center;">
          <div style="position:relative;flex:1;min-width:160px;">
            <svg
              style="position:absolute;left:10px;top:50%;transform:translateY(-50%);width:13px;height:13px;color:var(--ink-text-3);pointer-events:none;flex-shrink:0;"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
            ...
            <input
              ...
              [value]="filterSearch()"
              (input)="filterSearch.set($any($event.target).value); resetPage()"
            />
          </div>
          <select
            class="ink-select"
            ...
            (change)="filterCriticality.set($any($event.target).value); resetPage()"
          >
          ...
          <select
            ...
            (change)="filterCategory.set($any($event.target).value); resetPage()"
          >
          ...
          <select
            ...
            style="min-width:120px;"
            [value]="filterSource()"
            (change)="filterSource.set($any($event.target).value); resetPage()"
          >
        ...
        <div style="display:flex;gap:5px;flex-wrap:wrap;align-items:center;margin-bottom:8px;">
          <span
            style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);flex-shrink:0;"
            >Tags:</span
          >
          @for (tag of availableTags; track tag.value) {
            <button [class]="tagChipClass(tag.value)" (click)="toggleTagFilter(tag.value)">
              {{ tag.label }}
            </button>
          }
          @if (filterTags().length > 0) {
            <button
              ...
              (click)="filterTags.set([]); resetPage()"
            >
              ✕ Clear
            </button>
          }
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap;">
          ...
          <button
            ...
            [style.background]="
              sortField() === 'publish_score' ? 'rgba(99,102,241,.15)' : 'transparent'
            "
            [style.color]="
              sortField() === 'publish_score' ? 'var(--ink-brand)' : 'var(--ink-text-2)'
            "
          ...
          @if (pagedArticles().length === 0) {
            ...
            <div>
              ...
              <div
                ...
                style="align-items:center;gap:12px;padding:8px 16px;background:var(--ink-raised);margin-bottom:8px;"
              >
                ...
                <button
                  style="width:52px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:right;background:none;border:none;cursor:pointer;padding:0;font-family:'Outfit',sans-serif;"
                  (click)="setSort('publish_score')"
                >
                  Score {{ sortIcon('publish_score') }}
                </button>
                ...
                >
                <button
                  style="width:80px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:right;background:none;border:none;cursor:pointer;padding:0;font-family:'Outfit',sans-serif;"
                  (click)="setSort('created_at')"
                >
                  Date {{ sortIcon('created_at') }}
                </button>
                <span
                  style="width:56px;flex-shrink:0;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-text-3);text-align:right;"
                  >Act.</span
                >
              </div>
              ...
              <div style="display:flex;flex-direction:column;gap:6px;">
                @for (article of pagedArticles(); track article.id) {
                  ...
                  >
                    <!-- Checkbox -->
                    <div
                      style="flex-shrink:0;width:24px;padding-top:1px;"
                    ...
                    <div
                      ...
                      style="width:52px;flex-shrink:0;flex-direction:column;align-items:flex-end;gap:4px;"
                    >
                      <span
                        style="font-size:13px;font-family:'JetBrains Mono',monospace;font-weight:700;color:var(--ink-text);"
                      >
                    ...
                    <div style="flex:1;min-width:0;">
                      ...
                      <div
                        ...
                        style="align-items:center;gap:4px;flex-wrap:wrap;margin-bottom:4px;"
                      >
                        <span
                          class="ink-badge"
                          style="background:var(--ink-raised);color:var(--ink-text-2);"
                          >{{ countryFlag(article.country) }} {{ article.country }}</span
                        >
                      ...
                      <p
                        style="font-size:13px;font-weight:500;line-height:1.45;color:var(--ink-text);display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;"
                      >
                        {{ article.title }}
                      </p>
                      <div
                        style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:5px;font-size:11px;color:var(--ink-text-2);"
                      >
                        ...
                        <span
                          [class]="'ink-badge ' + criticalityBadgeClass(article.criticality)"
                          style="font-size:9px;text-transform:uppercase;letter-spacing:.05em;"
                          [title]="'Criticality: ' + article.criticality"
                          >{{ criticalityLabel(article.criticality) }}</span
                        >
                        @if (article.source_type === 'historical') {
                          <span
                            class="ink-badge"
                            style="font-size:9px;background:rgba(180,120,40,0.15);color:#b47828;letter-spacing:.05em;"
                            title="Historical / anniversary story"
                            >🏛️ {{ article.country === 'IT' ? 'STORIA' : 'HISTOIRE' }}</span
                          >
                        }
                        <span
                          [class]="'hidden sm:inline ink-badge ' + statusBadgeClass(article.status)"
                          >{{ article.status }}</span
                        >
                        @if (article.status === 'pending' && articleBestSlot(article)) {
                          <span
                            [class]="'ink-badge ' + intentBadgeClass(articleSlotIntent(article))"
                            style="font-size:9px;"
                            [title]="'Best slot for this article based on identity mode + criticality'"
                            >{{ articleSlotLabel(article) }}</span
                          >
                        }
                        <span
                          style="margin-left:auto;font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--ink-text-3);"
                          >{{ article.created_at | date: 'dd MMM' }}</span
                        >
                      </div>
                      @if (visibleTags(article).length > 0) {
                        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:5px;">
                          @for (tag of visibleTags(article); track tag) {
                            <span [class]="'ink-badge ' + tagBadgeClass(tag)">{{
                              tagLabel(tag)
                            }}</span>
                          }
                      ...
                      @if ((article.cluster_size ?? 1) >= 3) {
                        <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px;">
                          <span
                            class="ink-badge ib-alert"
                            title="{{ article.cluster_size }} sources covering this story"
                            >🔗 ×{{ article.cluster_size }}</span
                          >
                        </div>
                      }
                    </div>
            ...
            @if (totalItems() > pageSize) {
              ...
              >
                ...
                <span
                  ...
                  >{{ pageStart() }}–{{ pageEnd() }} / {{ totalItems() }}</span
                >
                <button
                  class="btn-ink"
                  style="height:32px;padding:0 12px;font-size:12px;"
    ...
    @if (selectedArticle()) {
      <div style="position:fixed;inset:0;z-index:50;" class="animate-fade-in">
        ...
        <div
          class="detail-panel"
          style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:min(700px,calc(100vw - 24px));max-height:90vh;border-radius:12px;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.6),0 0 0 1px rgba(255,255,255,.06);display:flex;flex-direction:column;"
        >
          <app-article-detail
...
export class ArticleListComponent implements OnInit, OnDestroy {
  ...
  filteredSorted = computed(() => {
    ...
    items.sort((a, b) => {
      const av = numericFields.includes(field)
        ? ((a as any)[field] ?? 0)
        : String((a as any)[field] ?? '');
      const bv = numericFields.includes(field)
        ? ((b as any)[field] ?? 0)
        : String((b as any)[field] ?? '');
  ...
  async batchPostToFacebook() {
    ...
    try {
      ...
      if (succeeded > 0) {
        this.showToast(
          `Posted ${succeeded} article${succeeded !== 1 ? 's' : ''} to Facebook${failed > 0 ? ` (${failed} failed)` : ''}`,
        );
        await this.load();
      } else {
  ...
  critColor(level: string): string {
    return (
      ({ breaking: '#ff3636', alert: '#ff8c00', trending: '#1e7aff', standard: '#00cc70' } as any)[
        level
      ] ?? '#1a2440'
    );
  }
