...
export async function publish(contentItem, channelConfig) {
  ...
  if (!pageId || !token) {
    throw new Error(
      `Missing FB credentials for envKey=${envKey} (FB_PAGE_ID_${envKey}, FB_ACCESS_TOKEN_${envKey})`,
    );
  }
  if (!contentItem.rendered_video_url) {
    throw new Error(
      `Content item ${contentItem.id} has no rendered_video_url. Run generate-reel first.`,
    );
  }
  ...
  if (contentItem.seo?.description) {
    ...
    const hashtagLine = (contentItem.hashtags || [])
      .map((h) => `#${h.replace(/^#/, '')}`)
      .join(' ');
  ...
  if (!caption) {
    throw new Error(
      `Content item ${contentItem.id} has no caption. Populate seo.description (Wild Eye) or ai_caption (news/pexels) before publishing.`,
    );
  }
