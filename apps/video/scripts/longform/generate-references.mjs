...
async function main() {
  ...
  const { data: refs, error: rErr } = await db
    .from('content_references').select('*').eq('project_id', projectId).neq('status', 'passed');
  if (rErr) throw new Error(rErr.message);
  if (refs.length === 0) { console.error('All references already passed. Barrier OPEN.'); return finish(); }

  console.error(`Generating ${refs.length} reference image(s), ${limit}-wide, model=${channel.imageModel}…`);

  await runPool(refs, async (ref) => {
    await db.from('content_references').update({ status: 'generating' }).eq('id', ref.id);

    for (let attempt = 1; attempt <= maxRetry + 1; attempt++) {
      const { jobId, url } = await generate(channel.imageModel, { prompt: ref.prompt, ...imgParams });
      ...
      if (gate.verdict === 'pass' || gate.verdict === 'error') {
        ...
        await db.from('content_references').update({
      ...
      if (gate.verdict === 'blocked') {
        await db.from('content_references').update({
          status: 'blocked', higgsfield_media_id: jobId, url, fail_reason: gate.reason,
        }).eq('id', ref.id);
        return { key: ref.key, verdict: 'blocked' };
      }
      // retry: regenerate unless we're out of attempts
      await db.from('content_references').update({
        status: 'failed', higgsfield_media_id: jobId, url, retry_count: attempt, fail_reason: gate.reason,
      }).eq('id', ref.id);
    }
...
async function finish() {
  const { data: all } = await db.from('content_references').select('key,status').eq('project_id', projectId);
