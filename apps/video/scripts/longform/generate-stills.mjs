...
async function main() {
  ...
  const { data: allRefs, error: rErr } = await db
    ...
    .select('key, url, higgsfield_media_id')
  ...
  if (!stills?.length) {
    console.error('No pending still rows to process.');
    return;
  }
  ...
  await runPool(
    ...
    async (clip) => {
      ...
      if (src === 'google') {
        console.error(
          `[skip] S${String(clip.scene_n).padStart(2, '0')} google — waiting for H3 workflow`,
        );
        counts.skipped++;
        return;
      }
      ...
      if (src === 'reference') {
        ...
        if (!ref?.url)
          throw new Error(
            `S${clip.scene_n}: reference route but no ref URL found (keys: ${clip.reference_keys})`,
          );
        if (dry) {
        ...
        await db
          .from('content_clips')
          .update({ clip_url: ref.url, status: 'generated' })
          .eq('id', clip.id);
      ...
      if (src === 'higgsfield') {
        if (dry) {
          ...
          console.error(
            `[dry][${model}] S${String(clip.scene_n).padStart(2, '0')} — ${refs.length} ref(s): ${refs.map((r) => r.key).join(', ') || 'none'}`,
          );
          counts.generated++;
          return;
        }
