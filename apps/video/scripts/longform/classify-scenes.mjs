...
if (!values.file)    { console.error('Error: --file <path> required');   process.exit(2); }
...
async function main() {
  ...
  if (values.dry) {
    let still = 0, clip = 0, text_card = 0;
    for (const e of entries) {
      const kind = dbKind(e);
      const src  = dbImageSource(e);
      console.log(`S${String(e.scene_n).padStart(2, '0')}  kind=${kind}${src ? `  image_source=${src}` : ''}`);
      if (kind === 'still')     still++;
  ...
  for (const e of entries) {
    ...
    if (error) {
      ...
      console.log(`  ✓ S${e.scene_n}: kind=${kind}${image_source ? ` image_source=${image_source}` : ''}`);
      updated++;
    }
  }
