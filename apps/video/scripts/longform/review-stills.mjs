...
async function rejectStill(id, reason) {
  ...
  if (!match) { console.error(`Invalid still id "${id}" — expected e.g. S07-B`); process.exit(2); }
  const scene_n = Number(match[1]);
  const cut = match[2].toUpperCase();

  const { data: row, error } = await db.from('content_stills')
  ...
  if (!row) { console.error(`No content_stills row for ${id} in project ${projectId}`); process.exit(1); }

  const { error: updErr } = await db.from('content_stills')
...
async function approveAll(act) {
  if (act == null) { console.error('--approve-all requires --act <number>'); process.exit(2); }
...
async function listStills(act) {
  let query = db.from('content_stills')
    .select('scene_n, cut, act, status, clip_url, fail_reason')
    .eq('project_id', projectId)
    .order('act').order('scene_n').order('cut');

  if (act != null) query = query.eq('act', Number(act));

  const { data: rows, error } = await query;
  if (error) throw new Error(error.message);
  if (!rows?.length) { console.log('No stills found.'); return; }
  ...
  for (const r of rows) {
    if (r.act !== currentAct) {
      ...
      actRows.forEach((x) => { if (x.status in counts) counts[x.status]++; });
