...
async function main() {
  ...
  const { data: refs } = await db.from('content_references').select('key, higgsfield_media_id, status').eq('project_id', projectId);
  const notReady = refs.filter((r) => r.status !== 'passed' || !r.higgsfield_media_id);
  if (notReady.length) {
    throw new Error(`⛔ Reference barrier CLOSED: ${notReady.length} reference(s) not passed — run Phase 3 first.`);
  }
  ...
  let q = db.from('content_clips').select('*').eq('project_id', projectId).eq('kind', 'clip')
  ...
  if (!clips.length) { console.error('No clips to generate (all done or none match).'); return finish(); }
  ...
  await runPool(clips, async (clip) => {
    ...
    for (let attempt = clip.retry_count + 1; attempt <= maxRetry + 1; attempt++) {
      ...
      try {
        ...
        if (!(clip.status === 'generating' && jobId)) {
          ...
          await db.from('content_clips').update({ status: 'generating', higgsfield_job_id: jobId }).eq('id', clip.id);
          clip.status = 'generating';
        }
        const { url } = await poll(jobId);
        await db.from('content_clips').update({ status: 'generated', clip_url: url, fail_reason: null }).eq('id', clip.id);
        return { scene: clip.scene_n, ok: true };
      } catch (err) {
        clip.higgsfield_job_id = null; clip.status = 'failed'; // force resubmit next attempt
    ...
    onSettle: (r) => {
      ...
      else console.error(`  ✗ scene ${r.value?.scene ?? '?'}: ${r.value?.reason ?? r.error?.message}`);
    },
  });
...
async function finish() {
  ...
  console.error(done ? '✅ All clips generated — Phase 5 (validation) may proceed.'
