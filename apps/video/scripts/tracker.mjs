...
const CSV_HEADER = 'id,title,format,slot,posted_at,views,watch_through_pct,follows,follows_per_1k,distribution,fb_multiplier\n';
...
if (!values.id || !values.views) {
  console.error(JSON.stringify({ error: 'missing-args', message: '--id and --views are required' }));
  process.exit(2);
}
...
if (!['organic', 'boosted', 'mixed'].includes(distribution)) {
  console.error(JSON.stringify({ error: 'invalid-distribution', message: 'distribution must be: organic | boosted | mixed' }));
  process.exit(2);
}
...
if (fetchErr) { console.error(JSON.stringify({ error: 'db-fetch', message: fetchErr.message })); process.exit(1); }
...
if (updErr) { console.error(JSON.stringify({ error: 'db-update', message: updErr.message })); process.exit(1); }
